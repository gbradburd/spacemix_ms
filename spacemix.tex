...
each allele sampled in population $k$ to have a probability $w$ ($0 \leq w_k \leq 0.5$ ) of being sampled from location $G^{\prime}_k$ 
and a probability $1-w$ of being sampled from location $G^{*}_k$, 
...
where $D$, dimension $2k \; \times \; 2k$, is a matrix of pairwise distances between all inferred locations (indexed by $\prime$) and all sources of admixture (indexed by *), and for readability, we denote, e.g., $F(D(G^{\prime}_i,\identifyadmixsource{G}_i))$, as $F(D_{i'\;,\;\identifyadmixsource{j}})$.
...
We proceed in our inference procedure as before, but now with additional locations and admixture proportions to infer.
...
\end{equation}
%
as specified by the parameters $w$, $\identifyadmixsource{G}$, $\vec{\alpha}$, and $\eta$, and the observed locations, $G$.  
We place a weak spatial prior on the sources of admixture, $\identifyadmixsource{G}$ around the centroid of the sampled locations, $G$. The admixture proportions, $w$, are capped at 0.5, to ensure identifiability,
...
\gb{MOVE TO SUPPMAT AND REFERENCE THE ISSUE IN DISCO}
