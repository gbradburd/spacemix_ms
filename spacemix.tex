...
%In addition, we introduce population-specific variance terms to accommodate unshared drift and sample size effects.  These are given by $I\bar{S_k}^{-1} + I\eta_k$, where $I$ is the identity matrix, $I\bar{S_k}$ is the mean sample size in population $k$ across all loci, and $\eta_k$ is the nugget estimated in population $k$.  
...
\end{equation}
where the constant of proportionality is the normalization constant of the posterior. $P(\widehat{\Omega} |\Omega (\vec{\alpha},\pmb{D}(\pmb{G'}) )$ is as before the Wishart likelihood of 
the sample covariance matrix of our standardized frequencies, and our priors on $\pmb{\vec{\alpha}}$ are as before. Here $P(\pmb{G'}) $ are the priors for the spatial locations, which we assume are independent across locations. For the prior for population $k$'s location $G'_k$ we use a bivariate normal spatial prior centered on the observed location $G_k$. We perform MCMC on all of the random variables and for more details on our Bayesian inference procedure see Appendix B. 
 
We view the sampled locations as the natural prior on where the populations are positioned under the null of isolation by distance (as well as a natural starting position for the MCMC). This prior also encourages the resulting geogenetic map formed to be anchored in the observed locations, and represent (informally) the minimum distortion to space necessary to satisfy constraints placed by genetic similarities of populations. However, one concern is that if the method returns a spatial configuration that strongly resembles a map that this could reflect the data not be strong enough to over come the prior, or the MCMC having not been run long enough. To overcome this concern we can also use random locations as the ``observed locations" ($G_k$) to remove any influence of the observed map on the output via the prior, or we can change the variance on the spatial priors to ascertain the effect of the prior on inference.
...
These statistics are in the spirit of both the $f_n$ statistics introduced by Reich et al (2009) and the admixture arrows in a TreeMix graph (Pickrell and Pritchard 2012), and are designed to pick up the signal of covariance between populations in excess of that predicted by our model.  These previous approaches use a null model of a branching population phylogeny and develop tests of excess covariance and violations of conditional independent of allele frequencies among populations that indicate that the population phylogeny model is violated in a manner consistent with admixture (gene flow) between disperate parts of the phylogeny. In place of a phylogeny ours tests use our spatial model of allelic covariance, to look for excess covariance between distant populations indicative of excess gene flow.
...
\gc{What do we want to say about the values of alpha used in matrix, I suspect we might want to average the frequencies over a number of draws of alpha from the posterior.}

We choose a focal population ($i$) that we wish to test. We remove this focal population from the dataset, and denote the dataset and covariance matrix where the $i^{th}$ population has been dropped with a ``$-i$'' in the subscript. We then predict the standardized allele frequencies at the geographic location ($G_i$) occupied by population $i$. To predict the standardized frequencies at  we use the mean frequency at $G_i$ under the conditional normal model. Given the parametric spatial covariance matrix $\Omega_{-i,-i}$, and the standardized allele frequencies $X_{-i}$ at all sampled geographic locations $G_{i}$, we calculate the conditional standardized observations at locus $\ell$ at location $G_i$ as
\begin{equation}
...
\end{equation}
such that $0 \leq f_{i,j} \leq 1$ and can be interpreted as a correlation coefficient between $i$ anf $j$ having regressed out our predicted frequencies at $i$.
\gc{Is this correct? maybe should be using mean here instead of expectation. I don't think you want these to be vectors }
This statistic calculates whether a focal population's deviations from its spatial predictions have excess covariance with another sampled population. Our $f_{i,j}$ is expected to be zero if our model has fully accounted for the covariance in allele frequencies between population $i$ and $j$. To identify significant departures from this expectation we can calculate a confidence interval for our f_{i,j} to see if it includes zero, by using a non-parametric bootstrap \gb{or jackknife???} across loci in the dataset (or block bootstrap for linked data).  \gb{Figure 3a shows the spatial \emph{f}-stat calculated between the admixed population and each other population.}
...
This procedure allows us to visualize the signature of excess covariance across the entire map by placing a spatial grid down. \gb{Figure 3b shows the spatial \emph{f}-stat calculated between the admixed population and each other population.}
...
\begin{figure}[ht!]
	...
	\caption{An illustration of the form of the admixed covariance given in Eqns. \ref{eq:admixed_covariance_1} and \ref{eq:admixed_covariance_2}.  Populations $i$ and $j$ are drawing admixture in proportions $p_i$ and $p_j$ from their respective sources of admixture, $i'$ and $j'$, and all pairwise spatial covariances (the $\omega$'s) are shown.  In this cartoon example, population $i$ is drawing more admixture from its source $i'$ than $j$ is from its source $j'$ (i.e., $p_i > p_j$).}\label{sfig:admixed_cov_diagram}
\end{figure}
...
Integral to the question of whether it constitutes a ring species in the purest  form of the idea is the issue of whether gene flow along the margins of the plateau has truly been continuous throughout the history of the expansion or if, alternatively, discontinuities in migration around the species complex's range have facilitated periods of differentiation in genotype or phenotype without gene flow (Mayr 1942, Coyne and Orr 2004).  At stake is whether the species offers an example of sympatric speciation (i.e. speciation with continuous gene flow).  Because the questions in this system are fundamentally both geographic and genetic in nature, it is eminently SpaceMix-able, and, within its spatial framework, we performed a number of analyses to investigate the geographic context of population differentiation in the greenish warbler species complex.
