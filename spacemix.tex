...
which is explictly co-estimated and depicted as arrows,
...
\end{equation}
where $\hat{f}_{\ell,k}$ is sample allele frequency at locus $\ell$ in population $k$, 
...
in which covariances between allele frequencies decays exponentially with a power of their distance
...
Note that if the standardized sample frequencies were Gaussian, then the sample covariance matrix would be a sufficient statistic,
...
and place priors on the geogenetic locations, $G$, that are independent across populations.  
...
We first apply the method several scenarios simulated using the coalescent simulator \textit{ms} \citep{Hudson2002}.  
Each scenarios is simulated using a stepping stone model in which populations are arranged on a grid with symmetric migration 
...
Because there is no sampled intervening population between admixed population 23 and its source of admixture 13, there is nothing to stop 23 from explaining its higher covariance with 13 via its chosen location $G_{(23)}$ rather than via that of its source of admixture \kadmixsource{(23)}.  
...
We used a random subset of 10,000 SNPs from the SNP dataset of \citet{Hellenthal}, which is comprised of 1,490 individuals sampled from 95 populations (see Figure \ref{sfig:empirical_maps}\subref{human_map} for map of sampling), as well as the latitude and longitude attributed to each population.  
...
\begin{figure}
	\centering
		...
		\subcaptionbox{Close-up of Eurasian populations \label{globe_eurasia_noad_map}}
...
\begin{figure}
	\centering
		...
		\subcaptionbox{Close-up of Eurasian populations \label{globe_eurasia_ad_map}}
...
Figures \ref{sfig:globe_ad_maps_realpr2} and \ref{sfig:globe_ad_maps_realpr3}, we show the results of other independent MCMC analyses on these data. The broad-scale patterns and results discussed above are consistent across these runs. However, as is to be expected, there is significant heterogeneity in the exact layout of sample and admixture locations. For example, there is some play, among MCMC runs, in the internal orientation of the African locations with respect to Eurasia.  Samples that draw a significant amount of admixture, such as the central Asian populations (Uygur, Hazara and Uzbekistani), switch their locations with that of their source of admixture (as was also seen across MCMC runs in the warbler data analysis). Similarly the Ethiopian and Ethiopian Jews choose locations, in some MCMC runs, close to the other North African samples, and draw admixture from near the Sub-Saharan samples (as do the other North African samples).
...
it also captures much more detail than is contained in the first two PCs (XXX ref Eurasia figure).
...
We would like to gratefully acknowledge Marjorie Weber, Will Wetzel, Yaniv Brandvain, Dan Runcie, Razib KhanBrad Shaffer, and the Coop Lab for helpful discussions.  We also thank Darren Irwin, Miguel Alcaide, Garrett Hellenthal, and Cristian Capelli for sharing data.
This work was supported in part by NSF grant \#1262645 (DBI) to PR and GC.
