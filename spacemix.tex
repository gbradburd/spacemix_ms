...
\plrm{haven't mentioned Bayesian anything yet}
...
\paragraph{Illustrated with Simulations.} To illustrate this inference procedure, we present Spacemix output for several scenarios simulated under the coalescent (using \textrm{ms}, (Hudson)).  In all scenarios, we simulate under a stepping stone model in which populations are arranged on a grid with symmetric nearest neighbor migration 
(Fig. \ref{sfig:lattice_scenarios}\subref{simple_lattice}), 
\plrm{periods in \latex get two spaces after; escape the space after to avoid this ugly behavior}
with 10 haploid individuals sampled from every other population at 10,000 unlinked loci (for details on all simulations, see Methods).  On all simulated datasets, we perform SpaceMix analyses in which we treat population locations as random variables to be estimated as part of the model, and randomly place the `observed' location of each population so as to remove the influence of the prior.  
