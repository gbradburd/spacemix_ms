source("spacemix_ms_sims.R")

my.wd <- "ms_spacemix/"
setwd(my.wd)
ms.files<-dir(my.wd,pattern="*Robj")
...
pdf(file=paste(my.wd,"treemix_trees_mig_",num.migs,".pdf",sep=""))
for(i in 1:length(ms.files)){
	ms.file<-strsplit(ms.files[i],".Robj")[[1]]
	plot_tree(paste(my.wd,"treemix_output/",ms.file,"_treemixed_m",num.migs, sep=""))
	mtext(ms.file)
}
dev.off()
...
for(i in 1:length(ms.files)){
	...
	pdf(file=paste(my.wd,ms.file,"treemix_fig_sequential.pdf",sep=""),width=12,height=8)
	...
	##plot first tree
	plot_tree(paste(my.wd,"treemix_output/",ms.file,"_treemixed", sep=""))
	##get ordered list of names of populations
	tmp<-read.table(paste(my.wd,"treemix_output/",ms.file,"_treemixed.treeout.gz",sep=""),nrow=1,as.is=TRUE)[1,]
	...
	plot_resid(paste(my.wd,"treemix_output/",ms.file,"_treemixed", sep=""),"temp.pops")	
	...
	for(num.migs in 1:3){
			...
			plot_tree(paste(my.wd,"treemix_output/",ms.file,  "_continuing_treemixed_m",num.migs, sep=""))
	}
	dev.off()
...
convert.ms.treemix<-function(file){
	print(paste(my.wd,file,".Robj",sep=""))
	## pops x SNPs 
	 show(load(paste(my.wd,file,".Robj",sep="")))
	...
	treemix.data<-cbind(treemix.data,temp) 	
	treemix.file<-  paste(my.wd,"treemix_", file,".out",sep="") 
