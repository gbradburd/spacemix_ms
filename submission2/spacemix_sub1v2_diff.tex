...
%DIF DEL /Users/gburd/Desktop/Dropbox/space.mix/ms/submission1/spacemix_plos_template.tex        Sun Jun 14 15:30:12 2015
%DIF ADD /Users/gburd/Desktop/Dropbox/space.mix/ms/submission2/spacemix_plos_template_sub2.tex   Mon Aug 24 10:25:07 2015
...
\newcommand{\secref}[1]{``\nameref{#1}''}
%DIF 155a155-158
\newcommand{\gb}[1]{{\it\color{magenta}{(#1)}}} %DIF > 
\newcommand{\plr}[1]{{\it\color{purple}{(#1)}}} %DIF > 
\newcommand{\gc}[1]{{\it\color{blue}{(#1)}}} %DIF > 
 %DIF > 
%DIF -------
...
We use genome-wide polymorphism data to build ``geogenetic maps\DIFdelbegin \DIFdel{'',}\DIFdelend \DIFaddbegin \DIFadd{,'' 
}\DIFaddend which, when applied to stationary populations, 
...
or approximations to the coalescent applied to sequence data \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Paul_Song2011, Li_Durbin2011}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Paul_Song2011, Li_Durbin2011, Schiffels_Durbin_2014}}%DIFAUXCMD
}\DIFaddend .
...
and reduced dimensionality representations of the data \DIFdelbegin \DIFdel{, such as Principal Components Analysis (PCA; }\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend e.g. \cite{cavallisforza1994, Patterson2006, price2006eigenstrat}).  
...
especially \DIFdelbegin \DIFdel{PCA (}\DIFdelend \DIFaddbegin \DIFadd{Principal Components Analysis
(PCA; }\DIFaddend also pioneered by Cavalli-Sforza \cite{menozzi1978synthetic}). 
...
\DIFdelbegin \DIFdel{PCA }\DIFdelend applied to human populations within continents\DIFaddbegin \DIFadd{, it }\DIFaddend often shows a close correspondence to geographic locations\cite{novembre_genes_2008,wang_quantitative_2012}.  
...
\DIFdelbegin \DIFdel{in some sense}\DIFdelend \DIFaddbegin \DIFadd{if the amount of drift since a common ancestor is small}\DIFaddend .
...
\DIFdelbegin \DIFdel{Within population }\DIFdelend \DIFaddbegin \DIFadd{Within-population }\DIFaddend variance may vary across samples due to either noise from a finite sample size or demographic history unique to that sample (e.g., bottlenecks or endogamy).  
...
so that calculating the likelihood of $\widehat \Omega$ is the same as calculating the \DIFdelbegin \DIFdel{likelihood }\DIFdelend \DIFaddbegin \DIFadd{probability }\DIFaddend of the data up to a constant.
...
\DIFaddbegin \DIFadd{This modeling approach rests on the assumption that the loci in the dataset are independent, that is, not in linkage disequilibrium (LD).
...
\paragraph{\DIFdelbegin \DIFdel{Illustrated with }\DIFdelend Simulations.} 
...
\DIFaddbegin \DIFadd{The axes of the resultant maps are presented as Northings and Eastings, 
...
\begin{centering}
{\bf{Figure 1}}
    Simulation scenarios and \DIFdelbegin \DIFdel{and }\DIFdelend their corresponding geogenetic maps estimated with SpaceMix.  
    The smaller circles in the simulation scenarios represent unsampled populations.  
    a) configuration of simulated populations on a simple lattice with spatially homogeneous migration rates \DIFaddbegin \DIFadd{(a plot showing the first two Principal Component axes for this simulation is given in \ref{sfig:sim_pc_maps}}\subref{lattice_pc}\DIFadd{)}\DIFaddend ; 
...
\DIFaddend In all scenarios, populations at the corners of the lattice are pulled in somewhat
because these have the least amount of data informing their relative placements\DIFdelbegin \DIFdel{.
In Fig.}\DIFdelend \DIFaddbegin \DIFadd{, and because, without nearest-neighbor migration from farther outside the lattice, they are in fact more closely related to their neighbors.  We also examined the effects of uneven sampling on inference by sampling random subsets of the simple lattice and barrier scenarios shown in Figs.}\DIFaddend \ \DIFdelbegin \DIFdel{\ref{sfig:sim_covariance_decays}, we show the relationship between genetic covariance, geographic distance, and inferred geogenetic distance for these simulations.}\DIFdelend \DIFaddbegin \DIFadd{\ref{sfig:lattice_scenarios}}\subref{simple_lattice} \DIFadd{and \ref{sfig:lattice_scenarios}}\subref{barrier_lattice}\DIFadd{.  The results of these analyses are given in Figs.\ \ref{sfig:uneven_sampling}}\subref{lattice} \DIFadd{and \ref{sfig:uneven_sampling}}\subref{barrier}\DIFadd{.
}\DIFaddend 
...
now, we model the covariance between \DIFdelbegin \DIFdel{$\hat X_{k,\ell}$ and $\hat X_{k,\ell}$}\DIFdelend \DIFaddbegin \DIFadd{$\hat X_{i,\ell}$ and $\hat X_{j,\ell}$}\DIFaddend , for each $\ell$, as
\begin{align}
...
The models described above may be used in various combinations.  In the simplest model, \DIFdelbegin \DIFdel{populations do not choose their own locations , nor are they allowed to }\DIFdelend \DIFaddbegin \DIFadd{locations are not estimated for populations , nor do they }\DIFaddend draw admixture; the only parameters to be estimated are those of the spatial covariance function given in equation \eqref{eq:spatial_covariance}, and the population-specific variance terms ($\eta_i$).  In the most complex model, population locations, the locations of their sources of admixture, and the proportions of admixture are all estimated jointly in addition to the parameters of the spatial covariance function and the population specific variances.  Users may wish to employ the more constrained models (e.g., fixing the locations or admixture proportions for some or all samples) in a model selection framework to test specific hypotheses.  \DIFaddbegin \DIFadd{We discuss the utility of these different models in the Appendix.
}\DIFaddend 

Allowing admixture gives sensible results for the scenario of Fig.\ \ref{sfig:corner_admix_scenarios}\subref{corner_admixture_lattice}\DIFdelbegin \DIFdel{:
}\DIFdelend \DIFaddbegin \DIFadd{;
}\DIFaddend in the resulting map,
...
and the admixed population (population 23) \DIFdelbegin \DIFdel{chooses }\DIFdelend \DIFaddbegin \DIFadd{draws }\DIFaddend admixture from very close to its true source (population 13), 
...
Here, the admixed population 18 is correctly identified as admixed\DIFdelbegin \DIFdel{, 
but it explains }\DIFdelend \DIFaddbegin \DIFadd{; 
however, }\DIFaddend its intermediate genetic relationships \DIFdelbegin \DIFdel{by taking a }\DIFdelend \DIFaddbegin \DIFadd{are explained through an estimated }\DIFaddend location close to its true admixture source (population 13)
\DIFdelbegin \DIFdel{, 
and drawing }\DIFdelend \DIFaddbegin \DIFadd{and source of }\DIFaddend admixture (95\% CI: 0.04--0.14) \DIFdelbegin \DIFdel{from a location }\DIFdelend on the far margin of the half of the grid on its own side of the barrier.
Because there is no sampled intervening population between admixed population 18 and its source of admixture 13, 
\DIFdelbegin \DIFdel{there is nothing to stop }\DIFdelend \DIFaddbegin \DIFadd{the model is able to explain population }\DIFaddend 18\DIFdelbegin \DIFdel{from explaining }\DIFdelend \DIFaddbegin \DIFadd{'s }\DIFaddend its higher covariance with \DIFaddbegin \DIFadd{population }\DIFaddend 13 via its \DIFdelbegin \DIFdel{chosen }\DIFdelend \DIFaddbegin \DIFadd{estimated }\DIFaddend location $G_{(18)}$\DIFaddbegin \DIFadd{, 
}\DIFaddend rather than via that of its source of admixture \kadmixsource{(18)}.  
...
\DIFaddbegin \DIFadd{As with the simulations described above, the axes of the geogenetic maps are presented
...
consisting of 95 individuals sampled at 22 distinct locations and sequenced at 2,334 SNPs, of which 2,247 were bi-allelic and retained for SpaceMix runs.  \DIFdelbegin \DIFdel{The libraries were prepared using a genotype-by-sequencing protocol and were run on an Illumina HiSeq 2000 with a paired-end sequencing protocol \mbox{%DIFAUXCMD
\cite{alcaide2014genomic}}%DIFAUXCMD
.}\DIFdelend \DIFaddbegin \DIFadd{These loci were treated as independent (i.e., un-linked).  We discuss other ways to accommodate linkage disequilibrium further in the Discussion.
}\DIFaddend 

We first ran SpaceMix on the population dataset, with no admixture. The resulting inferred map (Fig.\ \ref{sfig:warbler_pop}\subref{warb_pop_noad}) largely recapitulates the geography of the sampled populations around the ring.  The Turkish population (\textbf{TU}, \textit{Phylloscopus trochiloides} ssp.\ \textit{nitidus}) clusters with the populations in the subspecies \textit{ludlowi}, due to its recent expansion, but also \DIFdelbegin \DIFdel{chooses }\DIFdelend \DIFaddbegin \DIFadd{has }\DIFaddend a relatively high nugget parameter (see Fig.\ \ref{sfig:warb_pop_nugg}\subref{warb_pop_noad_nugg}), reflecting the population history it does not share with its \textit{ludlowi} neighbors.  In the \DIFdelbegin \DIFdel{North}\DIFdelend \DIFaddbegin \DIFadd{north}\DIFaddend , where the twin waves of expansion around the Tibetan Plateau are hypothesized to meet, the inferred geogenetic distance between populations from opposite sides of the ring was much greater than their observed geographic separation, reflecting the reproductive isolation between these adjacent forms (see Fig.\ \ref{sfig:warb_pop_distcomp}).  
%
\newline\newline
...
We then ran the method allowing admixture (Fig.\ \ref{sfig:warbler_pop}\subref{warb_pop_ad}). The only population sample with appreciable admixture is the Stolby sample (\textbf{ST}; $w=0.19$, 95\% credible interval: 0.146-0.238; Fig.\ \ref{sfig:warb_pop_adprop}).  This sample is known to be composed of an equal mixture of eastern \textit{plumbeitarsus} and western \textit{viridanus} individuals \cite{alcaide2014genomic}. Multiple runs agreed well on the level of admixture of the Stolby sample (see Fig.\ \ref{sfig:warbler_pop_compare}). What does vary across runs is whether the Stolby sample \DIFdelbegin \DIFdel{chooses to locate itself }\DIFdelend \DIFaddbegin \DIFadd{has an estimated location }\DIFaddend by the \textit{viridanus} cluster \DIFdelbegin \DIFdel{and draw }\DIFdelend \DIFaddbegin \DIFadd{while drawing }\DIFaddend admixture from near the \textit{plumbeitarsus} cluster\DIFaddbegin \DIFadd{, }\DIFaddend or vise versa; however, this is to be expected given the 50/50 nature of the sample's makeup (Fig.\ \ref{sfig:warbler_pop_compare}). The somewhat intermediate position of the Stolby sample, and its non-50/50 admixture proportion, likely partially reflect the influence of the priors (Fig.\ \ref{sfig:admix_prop_func_loc}). 
...
this increase in shared ancestry was accounted for by a large nugget for the \textit{nitidus} population\DIFdelbegin \DIFdel{;
but }\DIFdelend \DIFaddbegin \DIFadd{.
However, }\DIFaddend in the individual-level analysis, \DIFdelbegin \DIFdel{they cannot share a nugget parameter, 
and must therefore choose a location }\DIFdelend \DIFaddbegin \DIFadd{a nugget is estimated separately for each sample, 
so, the model must accommodate the much higher relatedness between this pair of individuals
through estimated locations that are }\DIFaddend close to each other and far from the rest of the samples.
...
The split between \textit{viridanus} and \textit{plumbeitarsus} individuals (blue and red, respectively), in the north at the contact zone of the two waves of expansion, is clearer now than in the population-based analysis, as \DIFaddbegin \DIFadd{the estimated locations of }\DIFaddend individuals from the Stolby population \DIFdelbegin \DIFdel{have moved to }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend near their respective clusters. 
\DIFdelbegin \DIFdel{Despite the fact that }\DIFdelend \DIFaddbegin \DIFadd{Although the geogenetic separation between the }\DIFaddend \textit{viridanus} and \textit{plumbeitarsus} individuals \DIFdelbegin \DIFdel{have moved away from each other in our geogenetic map}\DIFdelend \DIFaddbegin \DIFadd{is greater than their geographic separation}\DIFaddend , 
...
This horseshoe \DIFaddbegin \DIFadd{configuration}\DIFaddend , with \textit{viridanus} and \textit{plumbeitarsus} at its tips, is steady within and among runs of the MCMC and choice of position priors (see Figs.\ \ref{sfig:warb_ind_clouds}\subref{post_map_randpr2}-\ref{sfig:warb_ind_clouds}\subref{post_map_realpr2}).  
...
We therefore urge caution in the interpretation \DIFaddbegin \DIFadd{of }\DIFaddend our results, 
and view them as a simplistic but rich visualization of patterns of population structure.

We used \DIFdelbegin \DIFdel{a random subset of 10,000 SNPs from }\DIFdelend the dataset of Hellenthal \emph{et al.}\cite{Hellenthal}, 
...
\DIFaddbegin \DIFadd{In the analyses presented on human genotype data below, we have thinned the total dataset for LD in windows of 50 base-pairs, with a step-size of 5 base-pairs, and an upper limit of 0.2 on pairwise $r^2$ \mbox{%DIFAUXCMD
\cite{Purcell_etal_2007,plink}}%DIFAUXCMD
.  We then used a random subset of 10,000 SNPs to estimate the sample covariance.
}

\DIFaddend We ran two sets of SpaceMix analyses: in the first, we estimated population sample locations, 
...
which is unsurprising given the depth of recent attention on human admixture studies, particularly on the subset of \DIFdelbegin \DIFdel{HGDP samples }\DIFdelend \DIFaddbegin \DIFadd{these samples that are in the HGDP dataset
}\DIFaddend \cite{rosenberg_genetic_2002,li_worldwide_2008,Loh:13,patterson_ancient_2012,Hellenthal}. 
...
To investigate possible patterns of admixture further, we ran a SpaceMix analysis with admixture (results shown in Figs.\ \ref{sfig:globe_ad_maps} and \ref{globe_ad_props}). The biggest change between the geogenetic map of human populations inferred with admixture and that without is the positioning of African samples with respect to the rest of the world.  The relatively large geogenetic distances between these groups reflects the fact that Eurasian, North African, Oceanian, and American populations all share relatively large amounts of population history (and hence genetic drift) not shared with the Sub-Saharan African samples. Relative to the geogenetic map inferred without admixture, the inclusion of admixture \DIFdelbegin \DIFdel{allows samples that fall }\DIFdelend \DIFaddbegin \DIFadd{shifts the estimated locations of admixed samples }\DIFaddend intermediate between Sub-Saharan Africa and North Africa\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{/}\DIFaddend the Middle East \DIFdelbegin \DIFdel{to move closer to one }\DIFdelend \DIFaddbegin \DIFadd{toward one cluster }\DIFaddend or the other, which, in turn, \DIFdelbegin \DIFdel{allows }\DIFdelend \DIFaddbegin \DIFadd{pushes }\DIFaddend each of those major clusters to move relatively farther apart.  The Ethiopian and Ethiopian Jewish samples \DIFdelbegin \DIFdel{move to be }\DIFdelend \DIFaddbegin \DIFadd{have estimated locations }\DIFaddend closer to the Sub-Saharan samples than \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{those }\DIFaddend of the North African samples, but draw substantial amounts of admixture ($\sim 40\%$) from close to where the Egyptian sample has positioned itself in the the Middle East cluster, as do the Sandawe \cite{hodgson_early_2014,Pickrell:12}. 
...
Interestingly the Bantu South African sample, though it \DIFdelbegin \DIFdel{moves to join }\DIFdelend \DIFaddbegin \DIFadd{has an estimated location near }\DIFaddend the other Bantu samples, draws admixture from close to the San populations. This is consistent with previous signals of the expansion of Bantu-speaking peoples into southern Africa  \cite{Pickrell:12,Jakobsson_genomic_2012,Pickrell:14,Hellenthal}.  The inferred sample-specific drift parameters (the \DIFdelbegin \DIFdel{`nuggets'}\DIFdelend \DIFaddbegin \DIFadd{nuggets}\DIFaddend ) are similar between runs with and without admixture (Fig.\ \ref{sfig:globe_nuggs}).
%
\newline\newline
...
The other significant signal of admixture is between East and West Eurasia, a signal documented by many authors \cite{rosenberg_genetic_2002,li_worldwide_2008, xu_genome-wide_2008,Hellenthal}. The majority of samples maintain their relative positions within each of these groups; however, \DIFdelbegin \DIFdel{several of the populations whose inferred locations are intermediate }\DIFdelend \DIFaddbegin \DIFadd{there are several samples that show admixture }\DIFaddend between eastern and western Eurasia\DIFdelbegin \DIFdel{(in the SpaceMix analysis without admixture) now move towards one side and draw admixture from the other}\DIFdelend .  The Uzbekistani and Hazara samples \DIFdelbegin \DIFdel{move to be closer to }\DIFdelend \DIFaddbegin \DIFadd{have estimated locations close to }\DIFaddend the East Asian samples \DIFdelbegin \DIFdel{, while drawing }\DIFdelend \DIFaddbegin \DIFadd{and draw }\DIFaddend a substantial admixture proportion from close to where the Georgian and Armenian samples have located themselves\DIFdelbegin \DIFdel{, while conversely }\DIFdelend \DIFaddbegin \DIFadd{.  Conversely, }\DIFaddend the Uygur sample \DIFdelbegin \DIFdel{moves to be }\DIFdelend \DIFaddbegin \DIFadd{has an estimated location }\DIFaddend close to the Burusho, Kalash, and Pathan samples\DIFaddbegin \DIFadd{, and draws admixture from near the Mongola and Hezhen samples}\DIFaddend . The Tu sample (with a geogenetic location in East Asia) draws a small amount of ancestry from close to \DIFdelbegin \DIFdel{where the Uygur have positioned themselves. The Chuvash move close to }\DIFdelend \DIFaddbegin \DIFadd{the estimated location of the Uygur. The estimated location of the Chuvash sample is near the }\DIFaddend Russian and Lithuanian samples, \DIFdelbegin \DIFdel{drawing }\DIFdelend \DIFaddbegin \DIFadd{and the Chuvash draw }\DIFaddend admixture from close to the Yakut \DIFdelbegin \DIFdel{; the Turkishsample also draws a smaller amount of admixture from there}\DIFdelend \DIFaddbegin \DIFadd{(as do the Turkish, to a smaller extent)}\DIFaddend . There are several other East-West connections: the Russian and Adygei samples have admixture from a location ``north'' of the East Asian samples, and the Cambodia sample draws admixture from close to the Eygptian sample \cite{Treemix, Hellenthal}. 
...
There are a number of possible explanations for these results. As we only allow a single admixture arrow for each sample, populations with multiple, geographically distinct sources of admixture may be \DIFdelbegin \DIFdel{choosing }\DIFdelend \DIFaddbegin \DIFadd{have estimated }\DIFaddend admixture locations that average over those sources. This may be the case for the Hadza and Bantu Keynan samples \cite{Hellenthal}.  A second possibility is that the relatively steep prior on admixture proportion forces samples to \DIFdelbegin \DIFdel{choose }\DIFdelend \DIFaddbegin \DIFadd{draw }\DIFaddend lower proportions of admixture from locations that overshoot their true sources; this may explain the Xibo and Yakut admixture locations. A final explanation is that good proxies for the sources of admixture may not be included in our sampling, either because of of the limited geographic sampling of current day populations, or because of old admixture events from populations from which there are not other more directly descending modern populations. The admixture into the Indian and Pathan samples (whose admixture source also clusters with the Indian Jew samples in some MCMC runs) may be an example of this; Reich \emph{et al.} \cite{reich_india_2009} and Moorjani \emph{et al.} \cite{moorjani_india_2013} have hypothesized that many populations from the Indian subcontinent may be descended from an admixture event involving an ancestral Southern Indian population not otherwise represented in this dataset.

In Figs.\ \ref{sfig:globe_ad_maps_realpr2} and \ref{sfig:globe_ad_maps_realpr3}, we show the results of other independent MCMC analyses on these data. The broad-scale patterns and results discussed above are consistent across these runs. However, as is to be expected, there is significant heterogeneity in the exact layout of sample and admixture locations. For example, there is some play, among MCMC runs, in the internal orientation of the African locations with respect to \DIFdelbegin \DIFdel{Eurasia.  Samples }\DIFdelend \DIFaddbegin \DIFadd{the east-west axis within the Eurasian cluster.  For some samples }\DIFaddend that draw a significant amount of admixture, such as the central Asian populations (Uygur, Hazara and Uzbekistani), \DIFdelbegin \DIFdel{switch their location }\DIFdelend \DIFaddbegin \DIFadd{the estimated location switches }\DIFaddend with that of their source of admixture (as was also seen across MCMC runs in the warbler data analysis). Similarly the Ethiopian and Ethiopian Jew samples \DIFdelbegin \DIFdel{choose }\DIFdelend \DIFaddbegin \DIFadd{have estimated }\DIFaddend locations, in some MCMC runs, close to the other North African samples, and draw admixture from near the Sub-Saharan samples (as do the other North African samples).
...
\DIFaddbegin \DIFadd{Our method shows the utility of representing both isolation by distance and 
...
(e.g., \mbox{%DIFAUXCMD
\cite{Bookstein_1989,Sampson_Guttorp_1992,yang_model-based_2012,Petkova_2014_EEMS}}%DIFAUXCMD
).  
...
One concern is that the multiple admixed samples (from a single admixture event) may simply \DIFdelbegin \DIFdel{choose to cluster close to each other}\DIFdelend \DIFaddbegin \DIFadd{have clustered estimated locations}\DIFaddend , 
...
In this paper\DIFdelbegin \DIFdel{we have }\DIFdelend \DIFaddbegin \DIFadd{, we have treated the loci in the dataset as independent, and,
...
Additionally, although we have }\DIFaddend focused on the covariance among alleles at the same locus, 
\DIFdelbegin \DIFdel{but }\DIFdelend linkage disequilibrium (\DIFdelbegin \DIFdel{LD; }\DIFdelend covariance of alleles among loci) 
...
(2) \DIFdelbegin \DIFdel{populations may choose their own locations }\DIFdelend \DIFaddbegin \DIFadd{population locations are estimated}\DIFaddend , but not admixture; 
(3) populations may draw admixture, but their own locations are fixed; or
(4) \DIFdelbegin \DIFdel{populations may both choose their own locations and draw admixture }\DIFdelend \DIFaddbegin \DIFadd{population locations and admixture are both estimated}\DIFaddend .  
...
\begin{tabular}{| >{\centering\arraybackslash}m{6cm} | >{\centering\arraybackslash}m{3cm} | l |}
	...
	\DIFaddbeginFL \textbf{\DIFaddFL{(1)}} \DIFaddendFL stationary population locations, no admixture & $K + 3$	& $\alpha_0,\alpha_1,\alpha_2,\eta$	\\ \hline
	\DIFaddbeginFL \textbf{\DIFaddFL{(2)}} \DIFaddendFL inferred population locations, \hspace{0.5cm}no admixture & $2K + 3$	& $\alpha_0,\alpha_1,\alpha_2,\eta,G$	\\ \hline
	\DIFaddbeginFL \textbf{\DIFaddFL{(3)}} \DIFaddendFL stationary population locations, inferred admixture & $2K + 3$	& $\alpha_0,\alpha_1,\alpha_2,\eta,\identifyadmixsource{G},w$	\\ \hline
	\DIFaddbeginFL \textbf{\DIFaddFL{(4)}} \DIFaddendFL inferred population locations, inferred admixture & $3K + 3$	&$\alpha_0,\alpha_1,\alpha_2,\eta,G,\identifyadmixsource{G},w$	\\
	\hline
\end{tabular}
\caption{
...
\DIFaddbegin \DIFadd{Although we anticipate most empirical researchers will be interested in the joint inference of a geogenetic map with admixture (Model 4), we have presented these models separately, as we believe each have their own utility.  Model 1 and Model 3 can each be used to infer landscapes of allele frequencies, upon which genotyped individuals can be probabilistically placed (following \mbox{%DIFAUXCMD
\cite{Wasser2004}}%DIFAUXCMD
...
\DIFaddend Below, we outline the inference procedure for the most parameter-rich model (inference on both population locations, their sources of admixture, and the proportions in which they draw admixture, in addition to inference of the parameters of the spatial covariance function).
...
If the proposed move takes the parameter outside the range of its prior, the \DIFaddbegin \DIFadd{proposed }\DIFaddend move is rejected\DIFdelbegin \DIFdel{and we do not move in that iteration }\DIFdelend \DIFaddbegin \DIFadd{, in which case the current parameter value is sampled in that iteration and reused in the next iteration }\DIFaddend of the MCMC.
...
Populations \DIFdelbegin \DIFdel{A--D }\DIFdelend \DIFaddbegin \DIFadd{$A$--$D$ }\DIFaddend are arrayed  along a line, but there is recent admixture from \DIFdelbegin \DIFdel{D into B }\DIFdelend \DIFaddbegin \DIFadd{$D$ into $B$ }\DIFaddend (such that 40\% of the alleles assigned to B are sampled from location \DIFdelbegin \DIFdel{D}\DIFdelend \DIFaddbegin \DIFadd{$D$}\DIFaddend ).  
The lines show the expected covariance under isolation by distance that each population (\DIFdelbegin \DIFdel{A, C, or D}\DIFdelend \DIFaddbegin \DIFadd{$A$, $C$, or $D$}\DIFaddend , as indicated by line color) has with a putative population at a given distance.  The dots show the admixed covariance between \DIFdelbegin \DIFdel{B }\DIFdelend \DIFaddbegin \DIFadd{$B$ }\DIFaddend and the three other populations, as well as \DIFdelbegin \DIFdel{B}\DIFdelend \DIFaddbegin \DIFadd{$B$}\DIFaddend 's variance with itself (\DIFdelbegin \DIFdel{B-B}\DIFdelend \DIFaddbegin \DIFadd{$B$-$B$}\DIFaddend ) as specified by equation \eqref{eq:admixed_covariance_1}, with no nugget or sampling effect.
%
\newline\newline
{\bf{Figure 11}}    Lines show the covariances populations \DIFdelbegin \DIFdel{A, C, and D }\DIFdelend \DIFaddbegin \DIFadd{$A$, $C$, and $D$ }\DIFaddend would have with population \DIFdelbegin \DIFdel{B }\DIFdelend \DIFaddbegin \DIFadd{$B$ }\DIFaddend as a function of \DIFdelbegin \DIFdel{B}\DIFdelend \DIFaddbegin \DIFadd{$B$}\DIFaddend 's location,
    with no admixture,
    under the parametric form of equation \eqref{eq:spatial_covariance}.
    The colored dots above `\DIFdelbegin \DIFdel{B}\DIFdelend \DIFaddbegin \DIFadd{$B$}\DIFaddend ' show the covariances observed with \DIFdelbegin \DIFdel{B }\DIFdelend \DIFaddbegin \DIFadd{$B$ }\DIFaddend at that location if \DIFdelbegin \DIFdel{B }\DIFdelend \DIFaddbegin \DIFadd{$B$ }\DIFaddend has 40\% admixture from \DIFdelbegin \DIFdel{D}\DIFdelend \DIFaddbegin \DIFadd{$D$}\DIFaddend .
    There is no single spatial location with unadmixed covariances remotely similar to these.
\newline\newline

Due to its admixture from \DIFdelbegin \DIFdel{D, B }\DIFdelend \DIFaddbegin \DIFadd{$D$, $B$ }\DIFaddend has lower covariance with \DIFdelbegin \DIFdel{A }\DIFdelend \DIFaddbegin \DIFadd{$A$ }\DIFaddend than expected given its distance, somewhat higher covariance with \DIFdelbegin \DIFdel{C}\DIFdelend \DIFaddbegin \DIFadd{$C$}\DIFaddend , and much higher covariance with \DIFdelbegin \DIFdel{D}\DIFdelend \DIFaddbegin \DIFadd{$D$}\DIFaddend . In addition, the variance of \DIFdelbegin \DIFdel{B }\DIFdelend \DIFaddbegin \DIFadd{$B$ }\DIFaddend is lower than that of the other three populations, which each have variance $1$: the value of the covariance when the distance is zero. This lower variance results from the fact that the frequencies at $B$ represent a mixture of the frequency at $D$ and the frequency at $B$ before the admixture. 

Now, using this example scenario, let us return to the concern posed above: that admixture location and population location are not identifiable.  For the sake of simplicity, assume that we hold the locations of \DIFdelbegin \DIFdel{A, C, and D }\DIFdelend \DIFaddbegin \DIFadd{$A$, $C$, and $D$ }\DIFaddend constant, as well as the decay of covariance with distance (as could be the case if \DIFdelbegin \DIFdel{A-D }\DIFdelend \DIFaddbegin \DIFadd{$A$--$D$ }\DIFaddend are part of a larger analysis).  The covariance relationships of $B$ to the other populations cannot be simply satisfied \DIFdelbegin \DIFdel{by moving }\DIFdelend \DIFaddbegin \DIFadd{if }\DIFaddend $B$ \DIFdelbegin \DIFdel{towards D, as B }\DIFdelend \DIFaddbegin \DIFadd{had an estimated location near $D$, as $B$ }\DIFaddend would then have a covariance with \DIFdelbegin \DIFdel{C }\DIFdelend \DIFaddbegin \DIFadd{$C$ }\DIFaddend that is higher, and a covariance with \DIFdelbegin \DIFdel{A }\DIFdelend \DIFaddbegin \DIFadd{$A$ }\DIFaddend that is lower, than that we actually observe. 

Introducing admixture into the model allows it to satisfy all of these conditions: it can draw ancestry from \DIFdelbegin \DIFdel{D }\DIFdelend \DIFaddbegin \DIFadd{$D$ }\DIFaddend but keep part of its resemblance to \DIFdelbegin \DIFdel{A}\DIFdelend \DIFaddbegin \DIFadd{$A$}\DIFaddend , it avoids \DIFdelbegin \DIFdel{B having to move closer to C}\DIFdelend \DIFaddbegin \DIFadd{$B$ having an estimated location too close to $C$}\DIFaddend , and it explains \DIFdelbegin \DIFdel{B}\DIFdelend \DIFaddbegin \DIFadd{$B$}\DIFaddend 's low variance.  Even in the absence of a sample from population \DIFdelbegin \DIFdel{C, B }\DIFdelend \DIFaddbegin \DIFadd{$C$, $B$ }\DIFaddend is better described as a linear mixture of a population close to \DIFdelbegin \DIFdel{A and D}\DIFdelend \DIFaddbegin \DIFadd{$A$ and $D$}\DIFaddend .  However, there are specific scenarios in which a limited sampling scheme (both in size and location), can lead to tradeoffs in the likelihood between estimated population location and that of its source of admixture.
...
and the \DIFdelbegin \DIFdel{admixed populations are }\DIFdelend \DIFaddbegin \DIFadd{model is }\DIFaddend able to explain the high covariance \DIFdelbegin \DIFdel{they }\DIFdelend \DIFaddbegin \DIFadd{the admixed populations }\DIFaddend have with their sources of admixture via their inferred location,
rather than just via their inferred source of admixture and admixture proportion.  
The reason \DIFdelbegin \DIFdel{they choose to do explain their }\DIFdelend \DIFaddbegin \DIFadd{the model explains these admixed populations' }\DIFaddend anomalous covariance with their inferred location,
...
For each analysis,
\begin{itemize}
\item[1.] Five independent chains were run for $5\times 10^6$ MCMC iterations each in which \DIFdelbegin \DIFdel{populations were allowed to choose their own locations }\DIFdelend \DIFaddbegin \DIFadd{population locations were estimated }\DIFaddend (but no admixture).  Population locations were initiated at the origin (i.e.\, at iteration 1 of the MCMC, $G_i = (0,0)$), \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{or at uniformly distributed coordinates between the minimum and maximum observed range of latitude and longitude, and }\DIFaddend all other parameters were drawn randomly from their priors at the start of each chain.  
...
Finally, we compared the SpaceMix map to a map derived from a Principal Components Analysis \DIFdelbegin \DIFdel{(Patterson and Reich 2006)}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Patterson2006}}%DIFAUXCMD
}\DIFaddend .  For this analysis, we calculated the eigendecomposition of the mean-centered allelic covariance matrix, then plotted individuals' coordinates on the first two eigenvectors (e.g.\DIFdelbegin \DIFdel{\ Novembre et al 2008}\DIFdelend \DIFaddbegin \DIFadd{, \mbox{%DIFAUXCMD
\cite{novembre_genes_2008}}%DIFAUXCMD
}\DIFaddend ).  For consistency of presentation, we show the full Procrustes superimposition of the PC coordinate space around the geographic sampling locations.

\DIFaddbegin \subsection*{\DIFadd{Treemix Comparisons}}
\DIFadd{To contrast our approach to tree-based approaches to admixture we
applied }\texttt{\DIFadd{TreeMix}} \DIFadd{\mbox{%DIFAUXCMD
\cite{Treemix} }%DIFAUXCMD
to our spatial simulations.
%DIF > 
We took the }\texttt{\DIFadd{ms}} \DIFadd{output on which we had also run Spacemix 
(Figs.\ \ref{sfig:lattice_scenarios},\ref{sfig:corner_admix_scenarios},\ref{sfig:barr_inland_ad}), 
and converted it into }\texttt{\DIFadd{TreeMix}} \DIFadd{format.  
We also included an outgroup sequence for each dataset,
 ...
 We ran }\texttt{\DIFadd{TreeMix}} \DIFadd{without migration edges on the processed }\texttt{\DIFadd{ms.treemix.file.gz}} \DIFadd{file to construct the initial
tree, which was rooted using the }\texttt{\DIFadd{myoutgroup}} \DIFadd{sequence using the following command:
}\hspace*{\fill}\newline\hspace*{\fill}\newline
\texttt{\DIFadd{treemix -i ms.treemix.file.gz  -root myoutgroup -o treemix\_output}}
\hspace*{\fill}\newline\hspace*{\fill}\newline
\DIFadd{We then sequentially added admixture migration edges, 
using the following command to add another edge to the existing tree (``}\texttt{\DIFadd{prev.treemix}}\DIFadd{"):
}\hspace*{\fill}\newline\hspace*{\fill}\newline
\texttt{\DIFadd{treemix -i  ms.treemix.file.gz -root myoutgroup  -g  prev.treemix.vertices.gz  prev.treemix.edges.gz -m 1 -o  treemix\_output}}
\hspace*{\fill}\newline\hspace*{\fill}\newline
\DIFadd{The }\texttt{\DIFadd{TreeMix}} \DIFadd{graphs and residual covariance matrices were visualized
using the scripts provided with }\texttt{\DIFadd{TreeMix}}\DIFadd{. 
The code to process and plot the data is provided in the Appendix.}\\

\DIFadd{In Fig.\ \ref{sfig:treemix_lattice} we show the tree and admixture graphs produced when
}\texttt{\DIFadd{TreeMix}} \DIFadd{is run on a lattice stepping stone model 
(a smaller scale version of this exercise was previously done by Pickrell and Pritchard \mbox{%DIFAUXCMD
\cite{Treemix}}%DIFAUXCMD
). 
The tree produced by running }\texttt{\DIFadd{TreeMix}} \DIFadd{is rake-like, showing the lack of deep shared sub-division. 
...
(as was previously found \mbox{%DIFAUXCMD
\cite{Treemix}}%DIFAUXCMD
; see their Figure S14). 
...
but are unable to fully accommodate the spatial relationships between samples. 
 Different runs of }\texttt{\DIFadd{TreeMix}} \DIFadd{on the same dataset result in quite different trees and orders of migration events, 
...
we also present the results of }\texttt{\DIFadd{TreeMix}} \DIFadd{run on our expansion and barrier simulations.}\\

\DIFadd{In Figs.\ \ref{sfig:treemix_corneradmixture}, \ref{sfig:treemix_neighboradmixture}, and \ref{sfig:treemix_inlandadmixture},
we show the application of }\texttt{\DIFadd{TreeMix}} \DIFadd{to the scenarios simulated with admixture events.  
...
This reflects the fact that }\texttt{\DIFadd{TreeMix}} \DIFadd{has to add migration edges to cope with the residual
...
The poor performance of }\texttt{\DIFadd{TreeMix}} \DIFadd{here is the result of the 
spatially simulated data not conforming to the 
underlying assumption of the }\texttt{\DIFadd{TreeMix}} \DIFadd{tree-like model.
}
...
\DIFaddend \subsection*{\DIFdelbegin \DIFdel{simulated }\DIFdelend \DIFaddbegin \DIFadd{Simulated }\DIFaddend covariance decays}
...
\subsection*{\DIFdelbegin \DIFdel{warbler population nugget}\DIFdelend \DIFaddbegin \DIFadd{PCA maps of simulated datasets}\DIFaddend }
\DIFaddbegin \label{sim_pc_maps}
{\bf{Plots of the first two Principal Component axes (with variance explained labeled on the relevant axes)
			...
			\DIFadd{a) the basic lattice scenario shown in \ref{sfig:lattice_scenarios}}\subref{simple_lattice}\DIFadd{.
			b) the lattice scenario with admixture from Population 1 into Population 30, 
				shown in \ref{sfig:corner_admix_scenarios}}\subref{corner_admixture_lattice}\DIFadd{.
}

\subsection*{\DIFadd{The effects of uneven sampling}}
\label{uneven_sampling}
{\bf{The effect of uneven sampling on inference of geogenetic maps.}}
\DIFadd{a) inference using a dataset with a random subsample of 12 populations from the simple lattice scenario with nearest neighbor migration; b) inference using a dataset with a random subsample of 12 populations from the barrier scenario with nearest neighbor migration and a barrier down the center line of longitude.  Ellipses denote the 95\% credible interval on estimated geogenetic locations.
}

\subsection*{\DIFadd{Warbler population nuggets}}
\DIFaddend \label{warb_pop_nugg}
...
\subsection*{\DIFdelbegin \DIFdel{warbler }\DIFdelend \DIFaddbegin \DIFadd{Warbler }\DIFaddend population geographic/geogenetic distance comparison}
...
\subsection*{\DIFdelbegin \DIFdel{warbler }\DIFdelend \DIFaddbegin \DIFadd{Warbler }\DIFaddend population admixture proportions}
\label{warb_pop_adprop}
{\bf{Credible intervals on estimated warbler population admixture proportion parameters.}}

\subsection*{\DIFdelbegin \DIFdel{warbler }\DIFdelend \DIFaddbegin \DIFadd{Warbler }\DIFaddend population geogenetic map comparison}
...
\subsection*{\DIFdelbegin \DIFdel{warbler }\DIFdelend \DIFaddbegin \DIFadd{Warbler }\DIFaddend individual admixture proportions}
\label{warb_ind_adprops}
{\bf{Credible intervals on estimated warbler individual admixture proportion parameters.}}

\subsection*{\DIFdelbegin \DIFdel{warbler }\DIFdelend \DIFaddbegin \DIFadd{Warbler }\DIFaddend individual geogenetic map comparison}
...
\subsection*{\DIFdelbegin \DIFdel{warbler }\DIFdelend \DIFaddbegin \DIFadd{Warbler }\DIFaddend individual nuggets}
\label{warb_ind_nugg}
...
\subsection*{\DIFdelbegin \DIFdel{warbler }\DIFdelend \DIFaddbegin \DIFadd{Warbler }\DIFaddend individual pairwise-$\pi$}
\label{warb_ind_pwp}
...
\subsection*{\DIFdelbegin \DIFdel{warbler }\DIFdelend \DIFaddbegin \DIFadd{Warbler }\DIFaddend individual PCA map}
...
\subsection*{\DIFdelbegin \DIFdel{simulation }\DIFdelend \DIFaddbegin \DIFadd{Simulation }\DIFaddend scenario of populations on a line}
\label{line_scenario}
...
\DIFaddbegin \subsection*{\texttt{\DIFadd{TreeMix}} \DIFadd{analyses of Spatial Simulations: Lattice}}
\label{treemix_lattice}
{\bf{\texttt{TreeMix} analysis of lattice spatial coalescent simulation.}}  
\DIFadd{The tree, residual covariance matrix, and first three migration admixture arrows are shown.
}

\subsection*{\texttt{\DIFadd{TreeMix}} \DIFadd{analyses of Spatial Simulations: Barrier}}
\label{treemix_barrier}
{\bf{\texttt{TreeMix} analysis of lattice spatial coalescent simulation with a barrier.}}
\DIFadd{The tree, residual covariance matrix, and first three migration admixture arrows are shown.
}

\subsection*{\texttt{\DIFadd{TreeMix}} \DIFadd{analyses of Spatial Simulations: Expansion}}
\label{treemix_expansion}
{\bf{\texttt{TreeMix} analysis of lattice spatial coalescent simulation with an expansion event.}}
\DIFadd{The tree, residual covariance matrix, and first three migration admixture arrows are shown.
}

\subsection*{\texttt{\DIFadd{TreeMix}} \DIFadd{analyses of Spatial Simulations: Admixture}}
...
\subsection*{\texttt{\DIFadd{TreeMix}} \DIFadd{analyses of Spatial Simulations: Barrier and Short-Range Admixture}}
...
\subsection*{\texttt{\DIFadd{TreeMix}} \DIFadd{analyses of Spatial Simulations: Barrier and Mid-Range Admixture}}
...
\DIFaddend \subsection*{Warbler Sample Metadata}
...
We would like to gratefully acknowledge the Coop Lab, Yaniv Brandvain, Razib Khan, \DIFaddbegin \DIFadd{Joe Pickrell, 
Jonathan Pritchard, }\DIFaddend Dan Runcie, Brad Shaffer, Marjorie Weber, and Will Wetzel for helpful discussions.  
...
\DIFaddbegin \bibitem[Bookstein (1989)]{Bookstein_1989}
\DIFadd{Fred L. Bookstein.
}\newblock \DIFadd{Principal warps: thin-plate splines and the decomposition of deformations.
}\newblock \emph{\DIFadd{Pattern Analysis and Machine Intelligence, IEEE Transactions on}}\DIFadd{, 
11}\penalty0 \DIFadd{(6):}\penalty0 \DIFadd{567--585, 1989.
}

\DIFaddend \bibitem[Botigué et~al.(2013)]{botigue_gene_2013}
...
\DIFdelbegin %DIFDELCMD < \bibitem[Luca et~al.(1994)]{cavallisforza1994}
%DIFDELCMD < %%%
\DIFdel{Luca }\DIFdelend \DIFaddbegin \bibitem[Cavalli-Sforza et~al.(1994)]{cavallisforza1994}
\DIFadd{L. Luca Cavalli-Sforza}\DIFaddend , Paolo Menozzi, and Alberto Piazza.
...
\DIFaddbegin \bibitem[Purcell~(2009)]{plink}
\DIFadd{Shaun Purcell.
}\newblock {\DIFadd{PLINK v1.07}}\DIFadd{.
}\newblock \url{http://pngu.mgh.harvard.edu/purcell/plink/}\DIFadd{, 2009.
}

\DIFaddend \bibitem[Price et~al.(2006)]{price2006eigenstrat}
...
\DIFaddbegin \bibitem[Purcell et~al.(2007)]{Purcell_etal_2007}
\DIFadd{Shaun Purcell and Benjamin Neale and Kathe Todd-Brown and Lori Thomas and Manuel A.R. Ferreira and David Bender and Julian Maller and Pamela Sklar and Paul I.W. de Bakker and Mark J. Daly and Pak C. Sham.
}\newblock \DIFadd{PLINK: A Tool Set for Whole-Genome Association and Population-Based Linkage Analyses 
}\newblock \emph{\DIFadd{American Journal of Human Genetics}}\DIFadd{, 81}\penalty0 \DIFadd{(3):}\penalty0
\DIFadd{559--575, July 2007.
}

\DIFaddend \bibitem[Ralph and Coop (2013)]{ralph2013geography}
...
\DIFaddbegin \bibitem[Sampson and Guttorp (1992)]{Sampson_Guttorp_1992}
\DIFadd{Paul D.  Sampson  and  Peter  Guttorp.
}\newblock \DIFadd{Nonparametric Estimation of Nonstationary Spatial Covariance Structure.
}\newblock \emph{\DIFadd{Journal of the American Statistical Association}}\DIFadd{, 
87}\penalty0 \DIFadd{(417):}\penalty0 \DIFadd{108--119, 1992.
}

\bibitem[Schiffels and Durbin (2014)]{Schiffels_Durbin_2014}
\DIFadd{Stephan Schiffels  and Richard Durbin.
}\newblock \DIFadd{Inferring human population size and separation history from multiple genome sequences.
}\newblock \emph{\DIFadd{Nature Genetics}}\DIFadd{, 46}\penalty0 \DIFadd{(8):}\penalty0 \DIFadd{919--925, Aug 2014.
}\newblock \doi{10.1038/ng.3015}\DIFadd{.
}

\DIFaddend \bibitem[Schlebusch et~al.(2012)]{Jakobsson_genomic_2012}
...
\begin{figure}
	...
	\caption{
    Simulation scenarios and \DIFdelbeginFL \DIFdelFL{and }\DIFdelendFL their corresponding geogenetic maps estimated with SpaceMix.  
    The smaller circles in the simulation scenarios represent unsampled populations.  
...
%DIF > %%%%%%%%
%DIF > %% Make supplemental figure pages wider
%DIF > %%%%%%%%
\DIFaddbegin \setlength{\textwidth}{450pt}
...
\DIFaddend \begin{figure}
	\centering
		{\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=4.8in,height=8.8in]{../figs/sims/sim_covariance_decays.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=.8\textheight]{../figs/sims/sim_covariance_decays.pdf}\DIFaddendFL }
...
\begin{figure}
\centering
	\DIFaddbeginFL \subcaptionbox{\label{lattice_pc}}
	...
	\caption{\DIFaddFL{Plots of the first two Principal Component axes (with variance explained labeled on the relevant axes) 
			of the mean-centered covariance matrix from simulated spatial scenarios.
			a) the basic lattice scenario shown in \ref{sfig:lattice_scenarios}}\subref{simple_lattice}\DIFaddFL{.
			b) the lattice scenario with admixture from Population 1 into Population 30, 
				shown in \ref{sfig:corner_admix_scenarios}}\subref{corner_admixture_lattice}}\DIFaddFL{.
...
\centering
	\subcaptionbox{\label{lattice}}
		{\includegraphics[width=\textwidth]{../figs/sims/uneven_sampling_lattice.pdf}}
	\subcaptionbox{\label{barrier}}
			{\includegraphics[width=\textwidth]{../figs/sims/uneven_sampling_barrier.pdf}}
	\caption{\DIFaddFL{The effect of uneven sampling on inference of geogenetic maps. a) inference using a dataset with a random subsample of 12 populations from the simple lattice scenario with nearest neighbor migration; b) inference using a dataset with a random subsample of 12 populations from the barrier scenario with nearest neighbor migration and a barrier down the center line of longitude.  Ellipses denote the 95\% credible interval on estimated geogenetic locations.}}\label{sfig:uneven_sampling}
\end{figure}

\begin{figure}
\centering
	\DIFaddendFL \subcaptionbox{\label{warb_pop_noad_nugg}}
		{\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=4in,height=2.89in]{../figs/warblers/warb_pop_NoAd_nugget.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=.45\textheight]{../figs/warblers/warb_pop_NoAd_nugget.pdf}\DIFaddendFL }
	\subcaptionbox{\label{warb_pop_ad_nugg}}
			{\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=4in,height=2.89in]{../figs/warblers/warb_pop_Ad_nugget.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=.45\textheight]{../figs/warblers/warb_pop_Ad_nugget.pdf}\DIFaddendFL }
...
\begin{figure}
\centering
	{\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=6in,height=3in]{../figs/warblers/warb_pop_dist_compare.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{../figs/warblers/warb_pop_dist_compare.pdf}\DIFaddendFL }
	\caption{
...
\begin{figure}
\centering
	{\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=5in,height=3.6in]{../figs/warblers/warb_pop_adprop.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{../figs/warblers/warb_pop_adprop.pdf}\DIFaddendFL }
...
\begin{figure}
	\centering
		\subcaptionbox{\label{warb_pop_realpr1}}
			{\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1.85in,height=1.54in]{../figs/warblers/population_warbler_map_realpr1.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=.3\textheight]{../figs/warblers/population_warbler_map_realpr1.pdf}\DIFaddendFL }
		\subcaptionbox{\label{warb_pop_realpr2}}			
			{\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1.85in,height=1.54in]{../figs/warblers/population_warbler_map_realpr2.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=.3\textheight]{../figs/warblers/population_warbler_map_realpr2.pdf}\DIFaddendFL }
		\subcaptionbox{\label{warb_pop_randpr1}}
			{\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1.85in,height=1.54in]{../figs/warblers/population_warbler_map_randpr1.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=.3\textheight]{../figs/warblers/population_warbler_map_randpr1.pdf}\DIFaddendFL }
...
\begin{figure}
	\centering
		\subcaptionbox{Likelihood \label{admix_prop_func_loc_lnl}}
			{\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=6in,height=2in]{../figs/warblers/admix_prop_func_loc_lnl.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{../figs/warblers/admix_prop_func_loc_lnl.pdf}\DIFaddendFL }
		\subcaptionbox{Posterior probability \label{admix_prop_func_loc_prob}}
			{\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=6in,height=2in]{../figs/warblers/admix_prop_func_loc_prob.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{../figs/warblers/admix_prop_func_loc_prob.pdf}\DIFaddendFL }
...
\begin{figure}
\centering
	{\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=5in,height=2.5in]{../figs/warblers/warb_ind_adprop.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{../figs/warblers/warb_ind_adprop.pdf}\DIFaddendFL }
...
\begin{figure}
    ...
    \subcaptionbox{Warbler individuals map, no admixture} 
			{\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=2.8in,height=2.3in]{../figs/warblers/warb_ind_noad.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.4\textwidth]{../figs/warblers/warb_ind_noad.pdf}\DIFaddendFL }
            \subcaptionbox{Close-up of non-\textit{nitidus} samples}
			{\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=2.8in,height=2.3in]{../figs/warblers/warb_ind_noad_closeup.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.4\textwidth]{../figs/warblers/warb_ind_noad_closeup.pdf}\DIFaddendFL }
            \subcaptionbox{Warbler individuals map, admixture}
			{\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=2.8in,height=2.3in]{../figs/warblers/individual_warbler_map_arrows_randpr1.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.4\textwidth]{../figs/warblers/individual_warbler_map_arrows_randpr1.pdf}\DIFaddendFL }
            \subcaptionbox{Close-up of non-\textit{nitidus} samples}
			{\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=2.8in,height=2.3in]{../figs/warblers/individual_warbler_map_arrows_randpr1_closeup.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.4\textwidth]{../figs/warblers/individual_warbler_map_arrows_randpr1_closeup.pdf}\DIFaddendFL }
	\caption{
...
\begin{figure}
	\centering
		\subcaptionbox{random location prior \label{post_map_randpr2}}
			{\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=4in,height=2.88in]{../figs/warblers/warb_inds_ad_post_map_randpr2.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=.3\textheight]{../figs/warblers/warb_inds_ad_post_map_randpr2.pdf}\DIFaddendFL }
		\subcaptionbox{geographic location prior \label{post_map_realpr1}}
			{\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=4in,height=2.88in]{../figs/warblers/warb_inds_ad_post_map_realpr1.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=.3\textheight]{../figs/warblers/warb_inds_ad_post_map_realpr1.pdf}\DIFaddendFL }
		\subcaptionbox{geographic location prior \label{post_map_realpr2}}
			{\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=4in,height=2.88in]{../figs/warblers/warb_inds_ad_post_map_realpr2.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=.3\textheight]{../figs/warblers/warb_inds_ad_post_map_realpr2.pdf}\DIFaddendFL }
...
\centering
	\subcaptionbox{\label{warb_ind_noad_nugg}}
		{\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=5in,height=2.5in]{../figs/warblers/warb_ind_NoAd_nugget.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{../figs/warblers/warb_ind_NoAd_nugget.pdf}\DIFaddendFL }
	\subcaptionbox{\label{warb_ind_ad_nugg}}
		{\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=5in,height=2.5in]{../figs/warblers/warb_ind_Ad_nugget.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{../figs/warblers/warb_ind_Ad_nugget.pdf}\DIFaddendFL }
...
\begin{figure}
\centering
	{\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=5in,height=4.3in]{../figs/warblers/warb_ind_pairwise_pi.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{../figs/warblers/warb_ind_pairwise_pi.pdf}\DIFaddendFL }
...
\begin{figure}
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=4in,height=3.33in]{../figs/warblers/warb_ind_PC_map.pdf}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{../figs/warblers/warb_ind_PC_map.pdf}
	\DIFaddendFL \caption{The map of warbler individuals derived from a Principal Components analysis.  The PC coordinates have undergone a full Procrustes transformation around the actual sampling coordinates.}
...
\begin{figure}
	\centering
		\subcaptionbox{All population pairs \label{warb_ind_dist_compare_allpairs}}
			{\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=6in,height=3in]{../figs/warblers/warb_ind_dist_compare_allpairs.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{../figs/warblers/warb_ind_dist_compare_allpairs.pdf}\DIFaddendFL }
		\subcaptionbox{Just within population comparisons \label{warb_ind_dist_compare}}
			{\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=6in,height=3in]{../figs/warblers/warb_ind_dist_compare.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{../figs/warblers/warb_ind_dist_compare.pdf}\DIFaddendFL }
...
\begin{figure}
	\centering
		{\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=6in,height=2.5in]{../figs/globetrotter/globe_NoAd_dist_decay.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{../figs/globetrotter/globe_NoAd_dist_decay.pdf}\DIFaddendFL }
...
\centering
	\subcaptionbox{\label{globe_noad_nugg}}
		{\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=5in,height=2.5in]{../figs/globetrotter/globe_NoAd_nugget.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{../figs/globetrotter/globe_NoAd_nugget.pdf}\DIFaddendFL }
	\subcaptionbox{\label{globe_ad_nugg}}
		{\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=5in,height=2.5in]{../figs/globetrotter/globe_Ad_nugget.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{../figs/globetrotter/globe_Ad_nugget.pdf}\DIFaddendFL }
...
\begin{figure}
	\centering
		\subcaptionbox{Inferred map of human populations \label{globe_all_ad_map_realpr2}}
			{\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth,height=0.71\textwidth]{../figs/globetrotter/globe_Ad_map_AfricaInset_realpr2.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=.45\textheight]{../figs/globetrotter/globe_Ad_map_AfricaInset_realpr2.pdf}\DIFaddendFL }
		\subcaptionbox{Close-up of Eurasian populations \label{globe_eurasia_ad_map_realpr2}}
			{\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth,height=0.71\textwidth]{../figs/globetrotter/eurasia_Ad_map_indproc_realpr2.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=.45\textheight]{../figs/globetrotter/eurasia_Ad_map_indproc_realpr2.pdf}\DIFaddendFL }
...
\begin{figure}
	\centering
		\subcaptionbox{Inferred map of human populations \label{globe_all_ad_map_realpr3}}
			{\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth,height=0.71\textwidth]{../figs/globetrotter/globe_Ad_map_AfricaInset_realpr3.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=.45\textheight]{../figs/globetrotter/globe_Ad_map_AfricaInset_realpr3.pdf}\DIFaddendFL }
		\subcaptionbox{Close-up of Eurasian populations \label{globe_eurasia_ad_map_realpr3}}
			{\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth,height=0.71\textwidth]{../figs/globetrotter/eurasia_Ad_map_indproc_realpr3.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=.45\textheight]{../figs/globetrotter/eurasia_Ad_map_indproc_realpr3.pdf}\DIFaddendFL }
...
\begin{figure}
	\centering
		{\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=5.5in,height=5.5in]{../figs/globetrotter/globe_PCA_map.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{../figs/globetrotter/globe_PCA_map.pdf}\DIFaddendFL }
...
\begin{figure}
	\centering
		{\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=6in,height=2.5in]{../figs/sims/example_acceptance_rates.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{../figs/sims/example_acceptance_rates.pdf}\DIFaddendFL }
...
\begin{figure}
	...
	\caption{\texttt{\DIFaddFL{TreeMix}} \DIFaddFL{analysis of lattice spatial coalescent simulation.  The tree, residual covariance matrix, and first three migration admixture arrows are shown}}\label{sfig:treemix_lattice}
\end{figure}

\begin{figure}
	\centering
		{\includegraphics[width=\textwidth]{../figs/sims/Treemix_comparison/ms_dataset_barriertreemix_fig_sequential.pdf}}
	\caption{\texttt{\DIFaddFL{TreeMix}} \DIFaddFL{analysis of lattice spatial coalescent simulation with a barrier..  The tree, residual covariance matrix, and first three migration admixture arrows are shown}}\label{sfig:treemix_barrier}
\end{figure}

\begin{figure}
	\centering
		{\includegraphics[width=\textwidth]{../figs/sims/Treemix_comparison/ms_dataset_expansiontreemix_fig_sequential.pdf}}
	\caption{\texttt{\DIFaddFL{TreeMix}} \DIFaddFL{analysis of lattice spatial coalescent simulation with an expansion event.  The tree, residual covariance matrix, and first three migration admixture arrows are shown}}\label{sfig:treemix_expansion}
\end{figure}

\begin{figure}
	\centering
		{\includegraphics[width=\textwidth]{../figs/sims/Treemix_comparison/ms_dataset_corner_admixturetreemix_fig_sequential.pdf}}
	\caption{\texttt{\DIFaddFL{TreeMix}} \DIFaddFL{analysis of lattice spatial coalescent simulation with a long-range admixture event.  The tree, residual covariance matrix, and first three migration admixture arrows are shown}}\label{sfig:treemix_corneradmixture}
\end{figure}

\begin{figure}
	\centering
		{\includegraphics[width=\textwidth]{../figs/sims/Treemix_comparison/ms_dataset_neighbor_admixturetreemix_fig_sequential.pdf}}
	\caption{\texttt{\DIFaddFL{TreeMix}} \DIFaddFL{analysis of lattice spatial coalescent simulation with a barrier and short-range admixture event.  The tree, residual covariance matrix, and first three migration admixture arrows are shown}}\label{sfig:treemix_neighboradmixture}
\end{figure}
...
\begin{figure}
	...
	\caption{\texttt{\DIFaddFL{TreeMix}} \DIFaddFL{analysis of lattice spatial coalescent simulation with a barrier and mid-range admixture event.  The tree, residual covariance matrix, and first three migration admixture arrows are shown}}\label{sfig:treemix_inlandadmixture}
\end{figure}
\DIFaddend 
