...
Our method uses genome-wide polymorphism data to build a ``geogenetic map''
...
% replacing "allele frequency" with just "genetic" since "frequency" makes people think they need to be estimating frequencies well.
...
We also analyze a global sampling of human populations, for which we largely recover the geography of our sampling, with support for significant histories of admixture in some samples.  The implementation of this method, available as an R package \plr{is this true?}
...
\plr{Notes and stylistic decisions:
Generally, I think that the method is generally presented how we came to it,
which isn't necessarily the most natural way for others to understand it.
That narrative is useful, but we can save it for the Haldane's Sieve post. 
I'd vote for saying "we want to build a map that reflects genetic relationships, which is geography distorted by demography"
rather than "infer the map" and, later, "by the way, this won't be quite the geographic map".

Also, I shy away from so much use of "we" and "our" because when I read it I think of ownership, which is offputting.
It also feels more personable and friendly, maybe, but on the balance I tend to read it the first way.
At least maybe ``our'' should only refer to things already introduced,
since when I read it I think that I should already know what it is.
I am not attached, though.

``Fig.'' or ``figure'' or ``Figure''?  SuppMat is Supplementary?
}
...
There are many different methods to learn how population structure and demographic processes have left their mark on patterns of genetic variation within and between populations.
...
although interpretation can be tricky
since the assumptions of a tree-based model
(unstructured populations that split at discrete points in time)
are rarely met. \plr{fair?}
...
as results are affecdted by sample sizes,
...
\plr{right, so maybe merge this last sentence with the first and put this at the start of the section, not the end}
...
Our data consist of $L$ unlinked bi-allelic single nucleotide polymorphisms (SNPs) sampled across $K$ populations, 
\plr{it's not ``our'' data, since we want the reader to apply it to their own data.}
...
\plrm{can we get rid of the hat on $\hat{f}$?}
% Our method models the covariance in these allele frequencies between populations and across loci.  % don't need to foreshadow already
...
We first compute standardized sample allele frequencies at locus $\ell$, by
\begin{equation}
  \label{eq:standardized_sample_freqs}
  \hat{X}_\ell = (\hat{f}_{\ell}  - \bar{f}_{\ell})/\sqrt{\bar{f}_{\ell}(1-\bar{f}_{\ell})}\text{,}
\end{equation}
\plrm{below you use $\hat X$ as a matrix; add in subscript $k$'s so that it's clear which way the matrix goes}
where $\hat{f}_{\ell}$ is the vector of sample allele frequencies at locus $\ell$ across all $K$ samples, and $\bar{f}_{\ell}$ is the sample size-weighted sample mean frequency across the $K$ samples at locus $\ell$.  
\plr{$\bar{f}$ is the global mean frequency, no? if unclear, formula comes in methods anyhow?}
...
We assume that our standardized sample frequencies are generated independently at each locus by a spatial Gaussian process.  
\plr{don't need to say ``Gaussian'' yet, as we're just parameterizing the covariance, which makes sense regardless}
...
\plrm{maybe not ``is'' but ``we model as''?}
\begin{equation}
\label{eq:spatial_covariance}
\Omega^{(P)}_{i,j} = \frac{1}{\alpha_0} \text{exp} \left(	-\left(\alpha_1D_{i,j} \right)^{\alpha_2} \right) \text{,}
\end{equation}
\plrm{how about \eqref{eq:spatial_covariance} could be ``for $i\neq j$'' and then \eqref{eq:spatial_covariance2} could be just for $\Omega_{i,i}$, without the $\delta$?}
where $D_{i,j}$ is the geographic distance 
\plrm{should this be ``geogenetic distance''?}
...
\plr{should be consistent: ``sample'' or ``population''?}
and represents any excess allele frequency variance not captured by the spatial model,
and $1/\bar{S_i}$ accounts for the variance introduced by sampling within the population.
Finally, $\bar{S}$ is a vector of mean sample sizes in each population, where $\bar{S}_k$ is the mean sample size across all loci in population $k$.  
\plr{maybe just ``sample size'' (not ``mean'') and discuss missing data in the Methods? otherwise, confusing without having mentioned missing data.}

\plr{Now, we can say the distribution of the sample covariance isn't known in general,
but the central limit theorem implies that if the number of loci is large, the sample covariance converges to Wishart.}
Given the assumption of multivariate normality for our standardized sample frequencies, 
it follows that the sample covariance of our standardized sample frequencies calculated across loci 
...
\plrm{this happens if each locus has variance 1, after normalization; a different normalization would give a different number of degrees of freedom}
We denote this by
\begin{equation}
\label{eq:wishart_dist}
P(\widehat{\Omega} \mid \Omega) = \mathcal{W}\left( L\widehat{\Omega} \mid \Omega,L \right)
\end{equation}
Therefore, the likelihood of the standardized sample frequencies across loci is the likelihood of our sample covariance matrix, as this contains all of the information about our parameters of interest.  
\gb{Graham I feel like there's a way to say this last sentence better, but I'm not sure exactly what you're trying to say.} 
\plr{I think you're wanting to say that if the data were Gaussian, then the sample covariance matrix is a sufficient statistic,
so that calculating the likelihood of $\widehat \Omega$ is the same as calculating the likelihood of the data.}
...
\plr{already said this last thing}
...
\plrm{shouldn't this go up in the intro, not in the middle of the results? Can skip directly to the next paragraph (with a different heading).}
...
To generate this map, we treat populations' geographic locations as random variables and estimate them as part of our Bayesian inference procedure.  
\plr{haven't mentioned Bayesian anything yet; explain.  also, seems funny to me to say we ``estimate random variables'' -- I think we ``estimate parameters'' or ``put priors on the parameters and evaluate their posterior distribution''.}
The set of coordinates where the populations were sampled is $G$, 
and we designate the set of location parameters (i.e.\ coordinates in the geogenetic map) for our populations as $G'$. 
\plrm{I think we can drop the notation for the true locations and use $G$ for $G'$.}
Our parametric covariance matrix $\Omega$, as given by equation \eqref{eq:spatial_covariance2}, is a function of the pairwise matrix of distances between these inferred locations, $D(G')$, and our $\vec{\alpha}$ and $\eta$ parameters.  We acknowledge this dependence by writing $\Omega(\vec{\alpha},{D}(G'),\eta)$.  We write the posterior probability as 
\begin{equation}
\label{eq:cyol_prob}
...
Our priors on the parameters that control the shape of the decay of covariance with distance $P(\vec{\alpha})$ are given in the Methods, as are those on $P(\eta)$.  We assume that the priors on the inferred, geogenetic locations, $P(G')$ are independent across populations.  Because the observed locations are a natural prior on where populations are positioned under the null of isolation by distance, we use a very weak prior on population $k$'s location parameter ($G'_k$) that is centered around the observed location $G_k$.
...
\plrm{periods in latex get two spaces after; escape the space after to avoid this ugly behavior}
on which we then test various admixture scenarios.
On all simulated datasets, we perform SpaceMix analyses in which we treat population locations as random variables to be estimated as part of the model, 
\plr{again, estimating random variables}
and randomly place the `observed' location of each population so as to remove the influence of the prior.  
\plr{calling these ``observed'' locations risks confusion with the Procrustes step: just say we center the priors at random locations?}
...
\begin{figure}
	...
	\caption{
    Simulation scenarios and SpaceMix inference.  a) configuration of simulated populations on a simple lattice with spatially homogeneous migration rates; b) inference of population locations under the scenario in \ref{sfig:lattice_scenarios}\subref{simple_lattice};  c) a lattice with a barrier along the center line of longitude, across which migration rates are reduced by a factor of 5; d) inference of population locations under the scenario in \ref{sfig:lattice_scenarios}\subref{barrier_lattice}; e) a lattice with recent expansion on the eastern margin; f) inference of population locations under the scenario in \ref{sfig:lattice_scenarios}\subref{expansion_lattice}.
    \plr{mention in caption that small dots are unsampled populations, and make barrier more obvious? also, what are ``inferences'' -- samples from the posterior?}
...
\plr{haven't said ``effective distance'' before\dots}
In the expansion scenario, in which all populations in the last five columns of the grid have expanded in the recent past from the nearest population in their row (Fig. \ref{sfig:lattice_scenarios}\subref{expansion_lattice}), 
\plr{how recently? all at once or gradually?}
...
We them simulated a long-distance admixture event, on the same grid,
...
This creates excess covariance over anomalously long distances, which is clearly difficult to accommodate by a two-dimensional geogenetic map.
...
It would therefore be of great utility to directly model the action of admixture on spatial patterns of genetic variation.
\plrm{No need to be coy, the reader already knows we do admixture.  Merge this with the next section.}
...
\plr{
It's unclear if you're talking about transformed frequencies or not here.
That said, I think that here we can actually talk about just plain (untransformed) frequencies:
since the global mean frequency is estimated the same regardless of admixture,
transforming and admixing commute,
so it's equivalent to say that the (untransformed) admixed frequencies are weighted averages of the original ones.
So, can delete ``mean standardized'' below.
}

To incorporate recent admixture,
we imagine that population $k$ draws the majority of its ancestry from $G^{\prime}_{k}$, but that a proportion $0 \leq w_k \leq 0.5$ of its ancestry comes from another location \kadmixsource{k}, which we refer to as its source of admixture. 
\plr{instead of ``draws ancestry'', say what this means unambiguously (including a note that admixture could be independent between loci or not)}
The mean standardized population allele frequency at locus $\ell$ in population $k$ 
is a weighted average of the standardized allele frequencies at the geographic location of the sampled population chooses for itself ($X_{\ell,k'}$) 
and those at the coordinates of the source ($X_{\ell,\identifyadmixsource{k}}$) from which the observed population draws admixture:
\begin{equation}
w_k X_{\ell,k'} + (1-w_k) X_{\ell,\identifyadmixsource{k}} . \label{eqn-admixedfreq}
\end{equation}
We can allow each of our populations to have an independent spatial source of admixture. The standardized frequencies $X_{\ell,k'}$ and $X_{\ell,\identifyadmixsource{k}} $ are 
defined as above.
% well defined, as, given the population standardized allele frequency covariance matrix, equation \ref{eq:spatial_covariance}, we can get the distribution of allele frequencies at any point in space simply by plugging its spatial coordinates into $\Omega(D(G^{\prime}))$.
\plr{make up notation for \eqref{eqn-admixedfreq} and use it in \eqref{eq:admixed_covariance_1}. or, well, \eqref{eq:admixed_covariance_1} is about the equivalent of $X$, so maybe \eqref{eqn-admixedfreq} should talk about $X$-equivalents.}
...
\plr{the superscript P was not there in \eqref{eq:spatial_covariance2}. error?}
\begin{alignat}{3}
...
The admixed covariance between samples $i$ and $j$, $\identifyadmixsource{\Omega_{i,j}}$ is then a function of all the pairwise spatial covariances between the locations of populations $i$ and $j$ and the points from which they draw admixture, $\identifyadmixsource{i}$ and $\identifyadmixsource{j}$.  Those spatial covariances in turn are a function of all combinations of pairwise distances between their locations: $G_i'$, $G_j'$, \kadmixsource{i}, and \kadmixsource{j}.  This parametric covariance form is illustrated in Figure \ref{sfig:admixed_cov_diagram}.
=======
where $\identifyadmixsource{i}$ and $\identifyadmixsource{j}$ are the sources from which populations $i$ and $j$ are drawing their admixture with proportions $w_i$ and $w_j$, 
\plr{$i*$ and $j*$ aren't sources, they are indices.  oh: now $\Omega^{(P)}$ is a $2n \times 2n$ matrix; need to say so, else we risk confusion, for instance that admixture locations have to be already sampled locations.}
and the spatial covariance function of population frequencies, $\Omega$, is given by \eqref{eq:spatial_covariance} (this lacks the sample-specific variance terms on the diagonal). 
\plr{Oh, ok.  I was confused about this distinction above.}
Note that we then reintroduce the nugget, $\eta_k$, and the sample size effect, $1/\bar{S_k}$, for each population, 
\plr{can just say something like ``as above''}
to model drift or excess variance in population $k$ on top of that predicted by the mixture of frequencies given by our spatial model.  As in equation \eqref{eq:spatial_covariance2}, $\delta_{i,j} $ is the indicator function $1$ if $i=j$, and $0$ otherwise. 

%parameterized by the pairwise geographic distances between each pair of populations, \emph{without} the population-specific variance terms on the diagonal (i.e., in equation \eqref{eq:admixed_covariance_1}, i.e.\ $\Omega_{i,j} = \frac{1}{\alpha_0} \text{exp} \left(	\left( \alpha_1D_{i,j} \right)^{\alpha_2} \right)$).  

\plr{delete this paragraph? or insert at start of previous one.}
The covariance between admixed samples $i$ and $j$, $\identifyadmixsource{\Omega_{i,j}}$ is then a function of all the pairwise spatial covariances between populations $i$ and $j$ and the points from which they draw admixture, $\identifyadmixsource{i}$ and $\identifyadmixsource{j}$.  Those spatial covariances in turn are a function of all combinations of pairwise distances between their locations: $G_i$, $G_j$, \kadmixsource{i}, and \kadmixsource{j}.  This parametric covariance form is illustrated in Figure \ref{sfig:admixed_cov_diagram}.
...
\end{equation}
%
as specified by the parameters $w$, $\identifyadmixsource{G}$, $\vec{\alpha}$, and $\eta$, and the observed locations, $G$.  We place the same priors as stated above on $G^{\prime}$, $\vec{\alpha}$, $\eta$, and we now specify priors on $w$ and $\identifyadmixsource{G}$.  
\plr{no need to repeat; just say ``proceed as before, with additional locations and admixture proportions to infer'' instead of text before this point}
We place a weak spatial prior on the sources of admixture, $\identifyadmixsource{G}$ around the centroid of the sampled locations, $G$. The admixture proportions, $w$, are capped at 0.5, to ensure identifiability, %prevent populations from swapping identities with their source of admixture, 
and are heavily weighted towards small values to be conservative with respect to admixture inference.  These priors are detailed in Table \ref{tab:param_prior_tab}.

\begin{figure}[htp!]
	\centering
	\includegraphics[width=2in,height=2in]{figs/admix_cov_fig.pdf}
	\caption{An illustration of the form of the admixed covariance given in equations \eqref{eq:admixed_covariance_1}.  Populations $i$ and $j$ (at locations $i^{\prime}$ and $j^{\prime}$) are drawing admixture in proportions $w_i$ and $w_j$ from their respective sources of admixture, $\identifyadmixsource{i}$ and $\identifyadmixsource{j}$, and all pairwise spatial covariances (the $\Omega$'s) are shown.  In this cartoon example, population $j$ is drawing more admixture from its source $\identifyadmixsource{j}$ than $i$ is from its source $\identifyadmixsource{i}$ (i.e., $w_j > w_i$).
    }
...
\plr{this paragraph shouldn't be so buried, I think. also, do we have anything useful to say about why one might want to run a more constrained model?}
The models described above may be used in various combinations.  In the simplest model, populations do not choose their own locations, nor are they allowed to draw admixture, and the only parameters to be estimated are those of the spatial covariance function given in equation \eqref{eq:spatial_covariance}, and the population-specific variance terms ($\eta_i$).  In the most complex model, population locations, the locations of their sources of admixture, and the proportions of admixture are all estimated jointly in addition to the parameters of the spatial covariance function and the population specific variances.  
...
the only population that draws substantial admixture is the one that is actually admixed (population 30), 
and it draws admixture from the correct location.
...
and in close to the correct amount ($\bar{w}_{(23)} = 0.05; 95\% \text{ credible interval} = 0.02-0.08$). 
\plr{maybe we should note that we aren't simulating under the model, so it's not so bad that 0.1 falls outside the credible interval. 
I guess if we simulated a continuous population and knew the right funcitonal form for the covariance then we should expect to get it right.}
...
Here, the admixed population 23 is the only population that chooses a non-negligible proportion of admixture, 
but rather than drawing this admixture from its true source (population 13), 
it draws admixture from a location on the far margin of the half of the grid on its own side of the barrier.  
The sampling scheme here illustrates more fully the example laid out in Figure \ref{sfig:toy_admixture}: because there is no sampled intervening population between admixed population 23 and its source of admixture 13, there is nothing to stop 23 from explaining its higher covariance with 13 via its chosen location $G_{(23)}^{\prime}$ rather than via that of its source of admixture $\identifyadmixsource{G}_{(23)}$.  
...
\plr{Is it possible the likelihood surface here has two humps, and it got stuck on the wrong one here?
Currently, it's overestimating the admixture proportion (as like 60\% instead of 40\%, and as a result, has to push the source population further away,
rather than keeping it on the right side of 23. Also, I wish this one worked, since then I wouldn't feel dishonest wanting to drop it since it's pretty similar to the first one.}
...
\begin{figure}[htp!]
    ...
    \plr{what are inferred admxiture strengths?}
...
\plr{say this more succinctly:}
To minimize the potential influence of the spatial prior on population locations, for each empirical analysis, we performed a SpaceMix run in which random, uniformly distributed locations between, for longitude, the minimum and maximum observed longitude, and, for latitude, the minimum and maximum observed latitude, were used as the prior on population locations (in addition to two runs for which the observed locations are used as priors).  
For clarity and ease of interpretation, we then present a full Procrustes superimposition of the inferred population locations ($G'$) and their sources of admixture ($\identifyadmixsource{G}$), using the observed latitude and longitude of the populations/individuals ($G$) to give a reference position and orientation.  As results were generally consistent across multiple runs for each dataset regardless of the prior employed, unless stated otherwise we present only the results from the `random' prior analyses.
