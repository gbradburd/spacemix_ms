...
{\bf{Fig 1.}}
    ...
    a) configuration of simulated populations on a simple lattice with spatially homogeneous migration rates (a plot showing the first two Principal Component axes for this simulation is given in \ref{sfig:sim_pc_maps}a); 
    ...
    d) the maximum \textit{a posteriori} (MAP) estimate from the posterior distribution of population locations under the scenario in \ref{sfig:lattice_scenarios}a; 
    e) MAP estimate of population locations under the scenario in \ref{sfig:lattice_scenarios}b;
    f) MAP estimate of population locations under the scenario in \ref{sfig:lattice_scenarios}c.
\end{centering}
\newline\newline
...
%
\newline\newline
{\bf{Fig 3.}} An illustration of the form of the admixed covariance given in Equation \eqref{eq:admixed_covariance_1}.  Populations $i$ and $j$ are drawing admixture in proportions $w_i$ and $w_j$ from their respective sources of admixture, $\identifyadmixsource{i}$ and $\identifyadmixsource{j}$, and all pairwise spatial covariances (the $F$'s) are shown.  In this cartoon example, population $j$ is drawing more admixture from its source $\identifyadmixsource{j}$ than $i$ is from its source $\identifyadmixsource{i}$ (i.e., $w_j > w_i$).
\newline\newline
...
%
\newline\newline
{\bf{Fig 5.}} Sampling maps of both empirical systems analyzed.  (a) greenish warbler subspecies distributions of all 22 sampled populations (breeding grounds), consisting of 95 individuals and colored by subspecies \cite{alcaide2014genomic}; (b) sampling map for human dataset, consisting of 1,490 individuals from 95 population samples \cite{Hellenthal}.
\newline\newline
...
%
\newline\newline
{\bf{Fig 6.}} Inferred population maps with population labels colored as in Fig\ \ref{sfig:empirical_maps}a: a) the map inferred with no admixture inference; b) the map inferred with admixture inference.
\newline\newline
...
%
\newline\newline
{\bf{Fig 7.}} Inferred maps for warbler individuals, colored by subspecies in an analysis with admixture inference.  a)map inferred with admixture; b) close-up of all non-\textit{nitidus} samples in the admixture map.
\newline\newline
...
\newline\newline
%
{\bf{Fig 8.}} Map of human samples, inferred without admixture.\\ (a) complete map; (b) close-up of Eurasian samples.
%
\newline\newline
...
%
\newline\newline
{\bf{Fig 9.}}	Map of human samples, inferred with admixture. (a) complete map; (b) close-up of Eurasian samples.
    Italicized labels denote locations of admixture sources,
    with darkness proportional to the amount of admixture.
\newline\newline
%
\newline\newline
{\bf{Fig 10.}}	Mean admixture proportions (and 95\% CIs) for each population sample.
\newline\newline
...
%
\newline\newline
{\bf{Fig 11.}}    Lines show the covariances populations $A$, $C$, and $D$ would have with population $B$ as a function of $B$'s location,
    with no admixture,
