...
\end{equation}
the mean centering acts to to reduce the covariance among populations in $X_{\ell}^{\prime}$ compared to $X_{\ell}$, and can induce negative covariance between distant populations (as they are often on either side of the mean). In addition the covariance matrix of our mean centered frequencies has rank $K-1$ rather than $K$, as any one of our mean centered frequencies could be written as a linear function of the others, and so is non-invertible.

To circumvent this problem we take the $QR$ decomposition of the matrix $T$, and form a projection matrix $\Psi= Q_{,-K}$. We write our Wishart likelihood as
\begin{equation}
