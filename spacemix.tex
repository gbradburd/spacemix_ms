...
and are therefore needed to explain the bulk of the variance \citep{NovembreStephens}. 
...
The recently introduced SPA approach \citep{Yangetal} also has this advantage, 
...
Although PCA can be interpreted in light of simple admixture events \cite{Gil}, 
and \citep{SPAmix} can locate the admixed ancestry of out of sample individuals,
neither approach explicitly models admixture between multiple geographically distant locations,
as SpaceMix does.
Assignment methods are designed to deal with many admixed samples, 
but they have no null spatial model for testing admixture.
We feel that an isolation by distance null model is more appropriate for testing for admixture, 
...
Along these lines, it is noticeable that our European samples draw little admixture from elsewhere \citep[a fact also noted by][ using a different approach]{Hellenthal}, 
...
holds rich information about the timing and source of admixture events \citep[e.g.][]{Chakaborti, ,Reich, Myers, Gravel} 
...
in which covariance in allele frequencies decays with distance in both space and in time \citep[see][]{Mattias}.  
