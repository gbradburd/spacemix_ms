...
%DIF ADD /Users/gburd/Desktop/Dropbox/space.mix/ms/submission2/spacemix_plos_template_sub2.tex   Thu Jul 30 17:31:42 2015
...
and reduced dimensionality representations of the data, such as Principal Components Analysis (PCA; e.g. \cite{cavallisforza1994, Patterson2006, price2006eigenstrat}).  
...
PCA applied to human populations within continents often shows a close correspondence to geographic locations\cite{novembre_genes_2008,wang_quantitative_2012}.  
...
\DIFaddbegin \gb{This modeling approach rests on the assumption that the loci in the dataset are independent, that is, not in linkage disequilibrium (LD).
...
or, in the empirical analyses, we have thinned the total dataset for LD, with a upper limit on $r^2$ of ...}
\DIFaddend 
...
\DIFaddbegin \gb{The axes of the resultant maps are presented as Northings and Eastings, 
...
\DIFdelbegin \DIFdel{In all scenarios, populations at the corners of the lattice are pulled in somewhat
because these have the least amount of data informing their relative placements.
In Fig.\ \ref{sfig:sim_covariance_decays}, we show the relationship between genetic covariance, geographic distance, and inferred geogenetic distance for these simulations.
}\DIFdelend 

\DIFaddbegin \gb{In all scenarios, populations at the corners of the lattice are pulled in somewhat
...
\DIFaddend We next simulated a long-distance admixture event on the same grid,
...
\DIFdelbegin \DIFdel{A more subtle simulated admixture scenario, with admixture proportion of 10\% across a geographic barrier, 
is shown Fig.\ \ref{sfig:barr_inland_ad}}%DIFDELCMD < \subref{barr_inland_ad_scenario}%%%
\DIFdel{.  
The resulting SpaceMix map (Fig.\ \ref{sfig:barr_inland_ad}}%DIFDELCMD < \subref{barr_inland_ad_inference}%%%
\DIFdel{), 
...
%DIFDELCMD < %%%
\DIFdel{Another difficult scenario is shown in Fig.\ \ref{sfig:barr_inland_ad}}%DIFDELCMD < \subref{big_barr_ad_scenario}%%%
\DIFdel{,
...
\DIFaddbegin \gb{As with the simulations described above, the axes of the geogenetic maps are presented
...
}\DIFdelend \DIFaddbegin \DIFadd{a nugget is estimated separately for each sample, 
}\gb{so, the model must accommodate the much higher relatedness between this pair of individuals
through estimated locations that are close to each other and far from the rest of the samples.}
\DIFaddend The same phenomenon seems to be at work in determining the locations of a pair of individuals, one identified as \textit{P. t. ludlowi} (Lud-MN3), one as \textit{P. t. trochiloides} (Tro-LN11), 
...
\DIFdelbegin \DIFdel{In this paper }\DIFdelend \DIFaddbegin \gb{In this paper, we have treated the loci in the dataset as independent, and,
...
\DIFadd{Additionally, although }\DIFaddend we have focused on the covariance among alleles at the same locus, 
...
\DIFaddbegin \subsection*{\gc{Treemix Comparisons}}
...
The code to process and plot the data is provided }\gb{by us?  in the appendix or something?}\DIFadd{.}\\
%DIF > 
...
\bibitem[Luca et~al.(1994)]{cavallisforza1994}
Luca, Paolo Menozzi, and Alberto Piazza.
