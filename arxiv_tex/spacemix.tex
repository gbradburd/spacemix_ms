...
or approximations to the coalescent applied to sequence data \citep{Paul_Song2011, Li_Durbin2011,Schiffels_Durbin_2014}.
...
and reduced dimensionality representations of the data (e.g. \citep{cavallisforza1994, Patterson2006, price2006eigenstrat}).  
...
especially Principal Components Analysis (PCA; also pioneered by Cavalli-Sforza \citep{menozzi1978synthetic}). 
...
applied to human populations within continents, it often shows a close correspondence to geographic locations \citep[e.g.][]{novembre_genes_2008,wang_quantitative_2012}.  
...
We also examined the effects of uneven sampling on inference by simulating a 9 x 9 population grid, 
...
The results of these analyses are shown in Figs.\ \ref{sfig:uneven_sampling_total_grid} through 
...
These loci were treated as independent (i.e., un-linked).  We discuss other ways to accommodate linkage disequilibrium further in the Discussion.
...
We used the dataset of \citet{Hellenthal}, 
...
In the analyses presented on human genotype data below, we have thinned the total dataset for LD in windows of 50 base-pairs, with a step-size of 5 base-pairs, and an upper limit of 0.2 on pairwise $r^2$ \citep{Purcell_etal_2007,plink}.  We then used a random subset of 10,000 SNPs to estimate the sample covariance.
...
which is unsurprising given the depth of recent attention on human admixture studies, particularly on the subset subset of these samples that are in the HGDP dataset \citep[see][for various global analyses]{rosenberg_genetic_2002,li_worldwide_2008,Loh:13,patterson_ancient_2012,Hellenthal}. 
...
and may be better suited to visualization with model-based clustering-based methods \citep{STRUCTURE} or TreeMix/MixMapper-like methods \citep{Treemix,lipson_mixmapper_2013}.  
...
PCA is an extremely helpful method for visualization of high-dimensional data used in exploratory data analysis \citep{Tukey1977}.
In application to genetic data, PCA can quickly identify problematic samples and major axes of variation.  
Since geography is a major cause of differentiation, the first one or two PC axes often correspond to geography \citep{novembre_interpreting_2008}.  
However, other factors such as genome rearrangements \citep{ma2012investigation} 
or other genealogical distortions \citep{mcvean_genealogical_2009} 
can outweigh, or become mixed with, geographic structure in the results. 
...
Notably, the ``map'' obtained from PCA depends, sometimes dramatically, 
...
If isolation by distance holds, then the two dimensions in which SpaceMix infers geogenetic positions 
...
This comparison is also nicely illustrated by the example in Fig.\ \ref{sfig:line_scenario}, and in Fig.\ \ref{sfig:uneven_sampling_grid_subsamp_diag}.
