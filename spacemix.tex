...
\end{equation} 
\plrm{subscript on $S$ and $\delta$ and $\eta$; write $1/S$ instead so it doesn't parse as a matrix}
...
Given the assumption of multivariate normality for our sample frequencies, it follows that the sample covariance of our standardized sample frequencies calculated across loci 
%  {($\widehat{\Omega} = \hat{f}\hat{f}^T$)} 
is Wishart distributed with degrees of freedom equal to the number of loci ($L$) across which the covariance is calculated.  \gb{P\&G confirm?}.
...
\begin{figure}
    ...
    \plr{Perhaps make figs 1--3 one figure, with six parts? These could be pretty small and still visible, it could fit on top third of a page?}
\end{figure}
...
\begin{figure}
	...
	\caption{
    Inferred maps for warbler individuals, colored by subspecies under analyses with and without admixture inference. a) map inferred without admixture; (b) close up of all non-\textit{nitidus} samples in non-admixture map; c) map inferred with admixture; d) closeup of all non-\textit{nitrides} samples in the admixture map.
...
There are, however, a number of obvious departures in the individual inferred geogenetic map from the observed map. 
\plr{I'd say the most obvious is the two TU samples running away.  What's up with that?}
The most obvious again is the clear split between \textit{viridanus} and \textit{plumbeitarsus} individuals in the north at the contact zone of the two waves of expansion.  This is clearer now than in the population-based analysis as individuals from the Stolby population have moved to near their respective \textit{viridanus} and \textit{plumbeitarsus} clusters. 
...
in which case the populations we would expect the populations to form a line, 
