...
As we only get to observe the sample frequencies and we standardize our allele frequencies using the sample mean our predicted admixture covariance matrix needs to be transformed to accomodate these sampling considerations. We can do this as before see methods XXXX. We again treat the likelihood of our sample covariance matrix as Wishart now given the parameteric covariance matrix $\Omega^{(A)}$ as specified by the parameters $\vec{p}$, $G_*$, $G$,$\vec{\alpha}$, and $\vec{\eta}$. We treat the location of the source of admixture for population $k$, $G_{k'}$, and the population's admixture proportion, $p_k$, as random variables and jointly estimated as part of our inference procedure. \gc{state form of priors.}
...
For these analyses, we used the dataset from Alcaide et al (2014), which consisted of 95 individuals sampled at 22 distinct locations and sequenced at 2,334 SNPs, of which 2,247 were bi-allelic and retained for SpaceMix analyses.  We ran analyses in which we (a) allowed populations to choose their own locations, and (b) allowed populations to draw admixture \emph{and} choose their own locations.  For each of these analyses, we used random points on a line as the `observed' population locations.  We then repeated these analyses, but treated each sequenced individual as a population.  All analyses were run for $1 \times 10^{8}$ MCMC generations, sampled at intervals of $1 \times 10^{5}$ generations to minimize the autocorrelation in the MCMC samples.  For clarity and ease of interpretation, we present a full Procrustes superimposition of the inferred population locations ($G'$) (and, for analyses in which they were included, their sources of admixture ($G^{*}$)), using the observed latitude and longitude of the populations/individuals to give a reference position and orientation.
