...
In order to model this covariance, we draw on previous work (Cavalli-Sforza \& Edwards, Nicholson et al, Coop et al, Pickrell \& Pritchard), and model the process of genetic drift as approximately Gaussian.  Briefly, if the frequency of allele $A$ in an ancestral population is $\epsilon_A$, then we model the frequency of that allele in a daughter population as follows:
\begin*{equation}
