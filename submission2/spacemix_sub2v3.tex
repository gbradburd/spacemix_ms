...
%DIF DEL /Users/gburd/Dropbox/space.mix/ms/submission2/spacemix_1st_resub.tex            Tue Oct 27 12:21:19 2015
%DIF ADD /Users/gburd/Dropbox/space.mix/ms/submission2/spacemix_plos_template_sub2.tex   Tue Oct 27 12:06:43 2015
...
\DIFaddend We also examined the effects of uneven sampling on inference by \DIFdelbegin \DIFdel{sampling random subsets of the simple lattice and barrier scenarios shown in Figs.
\ \ref{sfig:lattice_scenarios}}%DIFDELCMD < \subref{simple_lattice} %%%
\DIFdel{and \ref{sfig:lattice_scenarios}}%DIFDELCMD < \subref{barrier_lattice}%%%
\DIFdel{.  }\DIFdelend \DIFaddbegin \DIFadd{simulating a 9 x 9 population grid, 
and subsampling this total grid into a variety of subsets that had successfully greater `uneven-ness' of sampling.
We then compared the performance of PCA and SpaceMix on these unevenly subsampled datasets.
}\DIFaddend The results of these analyses are \DIFdelbegin \DIFdel{given }\DIFdelend \DIFaddbegin \DIFadd{shown }\DIFaddend in Figs.\ \DIFdelbegin \DIFdel{\ref{sfig:uneven_sampling}}%DIFDELCMD < \subref{lattice} %%%
\DIFdel{and \ref{sfig:uneven_sampling}}%DIFDELCMD < \subref{barrier}%%%
\DIFdel{.}\DIFdelend \DIFaddbegin \DIFadd{\ref{sfig:uneven_sampling_total_grid} through 
Figs.\ \ref{sfig:uneven_sampling_grid_subsamp_diag}.  As the number of samples removed from the total dataset increases, 
the maps produced by plotting Principal Component axis 1 (PC1) against PC2 diverge more and more from 
the true configuration of the samples (Fig.\ \ref{sfig:uneven_sampling_med_dist}).  The same is also true for SpaceMix, 
but to a much smaller extent, and in all scenarios SpaceMix produces geogenetic maps that are more 
faithful to the true configuration of the samples than those generated using PCA. 
}\DIFaddend 
...
\begin{align}
\label{eq:admixed_covariance_1}\DIFdelbegin %DIFDELCMD < \begin{split}
%DIFDELCMD < \identifyadmixsource{\Omega_{i,j}} = 
%DIFDELCMD <   (1-w_i)(1-w_j) F(D_{i\;,\;j\;}) &\\
%DIFDELCMD <   {} + w_i(1-w_j) F(D_{\identifyadmixsource{i},\;j\;})    &\\
%DIFDELCMD <   {} + w_j(1-w_i) F(D_{i\;,\;\identifyadmixsource{j}})    &\\
%DIFDELCMD <   {} + w_i w_j F(D_{\identifyadmixsource{i},\;\identifyadmixsource{j}})    &\\
%DIFDELCMD <   {} + \delta_{i,j} (\eta_i + 1 / \bar{S}_i) &
%DIFDELCMD < \end{split}
%DIFDELCMD < %%%
...
\DIFdelbegin \DIFdel{SpaceMix offers much of the flexibility of PCA -- }\DIFdelend \DIFaddbegin \paragraph{\DIFadd{Comparison to PCA}}
\DIFadd{SpaceMix, }\DIFaddend like PCA, \DIFdelbegin \DIFdel{it }\DIFdelend is well suited to describing population structure in a continuous fashion --
but it also has a number of advantages over PCA. 
\DIFdelbegin \DIFdel{When isolation by distance holds, the first (one or ) two PCs often correspond reasonably well to some simple rotation of latitude and longitude; 
however, these first two PCs explain a relatively small part of the total variance of the data}\DIFdelend \DIFaddbegin \DIFadd{PCA is an extremely helpful method for visualization of high-dimensional data used in exploratory data analysis \mbox{%DIFAUXCMD
\cite{Tukey1977}}%DIFAUXCMD
.
In application to genetic data, PCA can quickly identify problematic samples and major axes of variation.  
Since geography is a major cause of differentiation, the first one or two PC axes often correspond to geography \mbox{%DIFAUXCMD
\cite{novembre_interpreting_2008}}%DIFAUXCMD
.  
However, other factors such as genome rearrangements \mbox{%DIFAUXCMD
\cite{ma2012investigation} 
}%DIFAUXCMD
or other genealogical distortions \mbox{%DIFAUXCMD
\cite{mcvean_genealogical_2009} 
}%DIFAUXCMD
can outweigh, or become mixed with, geographic structure in the results}\DIFaddend . 
...
\DIFaddend These higher order PCs can be hard to interpret in empirical data (see discussion in the warbler section).
...
\DIFaddbegin \DIFadd{Notably, the ``map'' obtained from PCA depends, sometimes dramatically, 
on the choice of genomic region used and on the choice and distribution of samples. 
When a large number of related samples are included, 
the resulting PC axes display a combination of between-population and within-population structure, 
...
In \DIFdelbegin \DIFdel{comparison, if }\DIFdelend \DIFaddbegin \DIFadd{this way, the generality of PCA is also its weakness: 
...
is designed to visualize the relationships between samples induced by dispersal of ancestors across geography.  
Our explicit modeling of admixture is also helpful: in PCA, admixed individuals appear in intermediate locations, but are not distinguished from an individual from an intermediate, more distantly related population.
If }\DIFaddend isolation by distance holds, then the two dimensions in which SpaceMix infers geogenetic positions 
...
\DIFaddend The application of SpaceMix to humans nicely illustrates the utility of our approach: 
the first two PCs of this dataset resemble a \DIFdelbegin \DIFdel{boomerang }\DIFdelend \DIFaddbegin \DIFadd{triangle }\DIFaddend (Fig.\ \ref{sfig:globe_PCA_map}), 
...
This comparison is also nicely illustrated by the example in Fig.\ \ref{sfig:line_scenario}\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{, and in Fig.\ \ref{sfig:uneven_sampling_grid_subsamp_diag}.
}\DIFaddend 
...
\DIFdelbegin \DIFdel{Another strong advantage of SpaceMix over current methods }\DIFdelend \DIFaddbegin \paragraph{\DIFadd{Admixture Arrows}}
\DIFadd{One of the greatest strengths of SpaceMix }\DIFaddend is the introduction of admixture arrows. 
...
\DIFaddbegin \bibitem[Ma and Amos (2012)]{ma2012investigation}
\DIFadd{J~Ma and C~I Amos.
}\newblock \DIFadd{Investigation of inversion polymorphisms in the human genome using
  principal components analysis.
}\newblock \emph{\DIFadd{PLoS One}}\DIFadd{, 7}\penalty0 \DIFadd{(7), 2012.
}

\DIFaddend \bibitem[Mal\'{e}cot(1975)]{Malecot1975}
Gustave Mal\'{e}cot.
...
\newblock \emph{Nature Genetics}, 46\penalty0 (8):\penalty0 919--925, Aug 2014.
\newblock \doi{10.1038/ng.3015}.
...
\DIFaddbegin \bibitem[Tukey (1977)]{Tukey1977}
\DIFadd{John~W. Tukey.
}\newblock \emph{\DIFadd{Exploratory Data Analysis}}\DIFadd{.
}\newblock \DIFadd{Addison Wesley, Massachusetts, 1 edition edition, 1977.
}

\DIFaddend \bibitem[Wake and Schneider (1998)]{wake_schneider1998}
...
\DIFdelbegin %DIFDELCMD < \label{uneven_sampling}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{sfig:uneven_sampling_total_grid}
\DIFaddend {\DIFdelbegin %DIFDELCMD < \bf{Fig.\ \ref{sfig:uneven_sampling}. The effect of uneven sampling on inference of geogenetic maps.}%%%
\DIFdelend \DIFaddbegin \bf{Fig.\ \ref{sfig:uneven_sampling_total_grid}}
	\DIFadd{The effect of uneven sampling on inference of geogenetic maps.}\DIFaddend }
	\DIFdelbegin \DIFdel{a) inference using a dataset with a random subsample of
	12 populations from the simple lattice scenario with nearest neighbor migration; b)inference using a dataset with a random subsample of 12 populations from the barrier scenario with nearest neighbor migration and 
	a barrier down the center line of longitude.Ellipses denote the }\DIFdelend \DIFaddbegin \DIFadd{This figure shows the complete grid with corresponding PC map
	and SpaceMix geogenetic map 
	(ellipses denote }\DIFaddend 95\DIFaddbegin \DIFadd{\% credible intervals after a 50}\DIFaddend \% \DIFaddbegin \DIFadd{burn-in).  
...
{\bf{Fig.\ \ref{sfig:uneven_sampling_grid_subsamp1}}
	\DIFadd{The effect of uneven sampling on inference of geogenetic maps.}}
	\DIFadd{This figure shows the subsampling Scenario 1, 
...
{\bf{Fig.\ \ref{sfig:uneven_sampling_grid_subsamp2}}
	\DIFadd{The effect of uneven sampling on inference of geogenetic maps.}}
	\DIFadd{This figure shows the subsampling Scenario 2, 
	in which the lower left, top left, and top right squares are `protected,'
...
{\bf{Fig.\ \ref{sfig:uneven_sampling_grid_subsamp_diag}}
	...
	Also shown are the corresponding PC map and 
	SpaceMix geogenetic map 
	(ellipses denote 95\% }\DIFaddend credible \DIFdelbegin \DIFdel{interval on estimated geogenetic locations.}\DIFdelend \DIFaddbegin \DIFadd{intervals after a 50\% burn-in).
			}\DIFaddend 

\DIFaddbegin \label{sfig:uneven_sampling_grid_med_dist}
...
\DIFaddend \subsection*{Warbler population nuggets}
\label{warb_pop_nugg}
...
\begin{figure}
\centering
	\DIFdelbeginFL %DIFDELCMD < \subcaptionbox{\label{lattice}}
%DIFDELCMD < 		%%%
\DIFdelendFL {\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{../figs/sims/uneven_sampling_lattice.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{../figs/sims/total_grid.pdf}\DIFaddendFL }
	\DIFdelbeginFL %DIFDELCMD < \subcaptionbox{\label{barrier}}
%DIFDELCMD < 			%%%
\DIFdelendFL \DIFaddbeginFL \caption{\DIFaddFL{The effect of uneven sampling on inference of geogenetic maps.  
...
\begin{figure}
\centering
	\DIFaddendFL {\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{../figs/sims/uneven_sampling_barrier.pdf}%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{../figs/sims/grid_subsamp1.pdf}\DIFaddendFL }
	\caption{The effect of uneven sampling on inference of geogenetic maps.  
			\DIFdelbeginFL \DIFdelFL{a) inference using a dataset with a random subsample of 12 populations from }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{This figure shows }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{simple lattice scenario with nearest neighbor migration; b) inference using a dataset with a random subsample of 12 populations from }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{subsampling Scenario 1, 
			in which }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{barrier scenario with nearest neighbor migration }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{lower left, top left, }\DIFaddendFL and \DIFdelbeginFL \DIFdelFL{a barrier down the center line of longitude}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{top right squares are `protected,'
			and 2 samples are randomly removed from all other squares}\DIFaddendFL .
			\DIFdelbeginFL \DIFdelFL{Ellipses denote }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Also shown are }\DIFaddendFL the \DIFaddbeginFL \DIFaddFL{corresponding PC map and 
			SpaceMix geogenetic map 
			(ellipses denote }\DIFaddendFL 95\% credible \DIFdelbeginFL \DIFdelFL{interval on estimated geogenetic locations}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{intervals after a 50\% burn-in)}\DIFaddendFL .}\DIFdelbeginFL %DIFDELCMD < \label{sfig:uneven_sampling}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{sfig:uneven_sampling_grid_subsamp1}
\end{figure}

\begin{figure}
\centering
	{\includegraphics[width=\textwidth]{../figs/sims/grid_subsamp2.pdf}}
	\caption{\DIFaddFL{The effect of uneven sampling on inference of geogenetic maps.  
