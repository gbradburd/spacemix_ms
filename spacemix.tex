...
We then ran the method allowing admixture (Figure \ref{sfig:warbler_pop}\subref{warb_pop_ad}). The only population sample with appreciable admixture is the Stolby sample ($w=0.19$ (95\% credible interval: 0.146-0.238), the sample is know to be composed of an equal mixture of eastern \textit{plumbeitarsus} and western \textit{viridanus} individuals \citep{alcaide2014genomic}. Multiple runs agreed well on the level of admixture of the Stolby sample (see caption of Supplementary Figure \ref{sfig:warbler_pop_compare}). What does vary across runs is whether the Stolby sample chooses to locate itself by the \textit{viridanus} cluster and draw admixture from near the \textit{plumbeitarsus}  cluster or vise versa; however, this is to be expected given the 50/50 nature of the sample's makeup (Supplementary Figure \ref{sfig:warbler_pop_compare}). The somewhat intermediate position of the Stolby sample, and its non-50/50 admixture proportion, likely partially reflect the influence of the priors (Supplementary Figure \ref{sfig:warbler_pop_compare}). 
...
There are, however, a number of obvious departures in the individual inferred geogenetic map from the observed map.  The most obvious is that the location of a pair of \textit{nitidus} samples is very far from the rest of the samples.  
...
The split between \textit{viridanus} and \textit{plumbeitarsus} individuals, in the north at the contact zone of the two waves of expansion, is clearer now than in the population-based analysis as individuals from the Stolby population have moved to near their respective clusters. 
...
also allowed admixture. We note that few of the putative admixture events that we report have escaped the notice of previous investigators, which is unsurprising given the depth of recent attention on human admixture studies (particularly on the subset of HGDP samples). However, what is novel is the ability to vizualize these events in a geographic context.
...
The patterns of genetic variation observed in modern populations are the product of a complex history of demographic processes.  We choose to model those patterns as the outcome of a spatial process of spatially determined migration, 
...
Aspects of population history that are better described as a population phylogeny, will lead to difficult to interpret results using Spacemix 
(and may be better suited to visualization with clustering-based methods or treemix).  
There is obviously no one best approach to studying and visualization population structure, 
and that investigators should try and range of appropriate methods to identify those that provide insight. \gc{this prolly needs to be less preachy}
...
Like PCA our method is well suited to describing population structure in a continuous fashion. 
Our approach offers much of the flexibility of PCA, but with a number of advantages. 
...
however, these first two PCs explain a relatively small part of the variance. Due to the linearity of PCs, many higher order PCs 
have to correspond to higher order functions of positional information under isolation by distance, 
and are needed to explain the bulk of the variance \citep{NovembreStephens}. 
These higher order PCs can be hard to interpret (see discussion in the warbler section).
In our approach, if isolation by distance holds then (nearly) all of the variance will be captured by our infered geogenetic positions (to the extent to which our parametric form of the covariance is flexible enough to capture the empirical decay of covariance with distance). 
The recently introduced SPA approach \citep{blah} also has this advantage, 
but only really makes use of loci that conform to particular spatial patterns so it is necessarily making use of less of the data and is less flexible (although we note that PCA and SPA both have significant speed advantages over our approach).  
The application of our approach to humans nicely illustrates the utility of our approach, the first two PCs of this dataset resemble a boomerang, with its arms corresponding to the Africa/Non-Africa split and the spread of populations across Eurasia. In contrast, while our map is dominated by the out of Africa drift, it also contains much more detail than is contained in the first two PCs (ref Eurasia figure). An advantage of PCA is that can explain more complex patterns of populaiton structure by allowing up to $K$ different axes, our approach can easily be extended to more than two dimensions, simply by allowing $G_i$ to describe the location of a sample in $d$ dimensions. However, the interpretation and visualization of these higher dimensions would prove difficult, and so for the moment we stick with two dimensions.
...
Another strong advantage of our approach over PCA is our introduction of admixture arrows. PCs can be intepreted in light of simple admixture events \cite{Gil}, 
while \citep{SPAmix} locates the admixed ancestry of out of sample individuals. In contrast, 
our approach explicitly models admixture between multiple geographically distant locations.  
Assigment methods are explicitly designed to deal with many admixed samples, however, 
we feel that an isolation by distance null model is more appropriate for testing for admixture when we have many geographically densely samples. 
As such, Spacemix offers a useful tool to understand and visualize spatial patterns of genetic relatedness when many samples are admixed. 
...
In addition we've assume that only single populations are admixed, when in fact it is likely that particular admixture events may affect multiple samples.
...
Along these lines, it is noticible that our European samples draw little admixture from elsewhere \citep[a fact also noted by][ using a different approach]{Hellenthral}, 
despite evidence of substantial admixture \citep{Reichs3europeanpaper}, 
...
However, this may also simply reflect the fact that admixture is ancient, and the ancient individuals used as proxy sources of these 
events are not well respresented by our extant samples. Reassuringly, we see multiple cases where similarly admixed populations (Central Asians, Middle Easten and North African) populations 
are separately identified as admixed. This suggests that this is not a huge concern (at least in some cases). 
...
In this paper we have resticted our attention on the covariance among alleles at the same locus, 
however, linkage disequilibrium (covariance of alleles among loci) holds rich information about the timing and source of admixture events \citep[e.g.][]{Chakaborti, ,Reich, Myers, Gravel} 
as well as information about isolation by distance \citep{RalphandCoop}.
Just as population graph approaches have been extended to incorporate information from LD \citep{Mixmapper, others}, 
a spatial covariance approach could be taken to LD. A null model inspired by models of LD under isolation by distance models \citep{blah} 
could be fitted, allowing the covariance among alleles to decay with their geographic distance and the recombination distance between the loci. Sources and time-scales of admixture could be identified through unusually long-distance LD among geographically separated populations. 

The landscape of allele frequencies on which the samples, and sources of admixture for the sampled populations, are estimated is entirely informed by the placement of other modern samples.  
This immediately leads to the caveat that instead of ``location of the parental population'',
we should instead refer to ``location of the closest descendants of the parental population''.
