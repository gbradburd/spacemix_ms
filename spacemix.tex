...
In Figure \ref{sfig:globe_ad_maps} we show the results of of the location and admixture analysis of the human population dataset.  The opacity \gc{THIS IS NOT A WORD} of the admixture arrows (which point from the estimated location of a population's source of admixture to its own estimated location) is proportional to the weight of admixture that population draws, which means that some arrows are hard to see.  These faint signatures of admixture seem nonetheless to be biologically meaningful: for example the Maya draw admixture from a location close to the North African and Middle Eastern samples of only 7.3\% (X - X), a finding similar to that of Patterson \textit{et al.} (2012) and Hellenthal \textit{et al.} (2014) that reflects the combination of ancestry they received from North Africa and Spanish Europeans.

The biggest change between the geogenetic map of human populations inferred with admixture and that without is the positioning of Sub-Saharan African samples with respect to the rest of the world.  The inclusion of admixture allowed samples that fell intermediate between Sub-Saharan Africa and North Africa and the Middle East to move closer to one or the other, which, in turn, allows each of those major clusters to move relatively farther apart.  The Ethiopian and Ethiopian Jewish samples move to be closer to the Sub-Saharan samples than the North African samples, but draw substantial amounts of admixture ($\sim 40\%$) from close to where the Egyptian sample has positioned itself in the the middle East cluster, as do the the Hadza and the Sandawe.  

Reciprocally, the North African populations (Egyptian, Tunisian, Morocan, Mozabite) move to join the Middle Eastern populations (positioning in rough accord with their sampling location along North Africa), and draw admixture from near the Sub-Saharan Africa samples.  Interestingly the Bantu South African sample, though it moves to join the other Bantu populations, draws admixture from close to the San populations. This is consistent with previous signals of the expansion of Bantu-speaking peoples into southern Africa (Pickrell et al 2012; Hellenthal 2014).
...
SanKhomani draws admixture from near syria.  Hellenthal et al infer 2 admixture events and 3 admixture sources for the San Khomani: 1 event from the Bantu peoples in South Africa, and 1 event from a combination of northern European and southeast Asian populations.  If you squint, the centroid of these locations is the middle east, from which the San Khomani choose to draw their admixture in our analysis.

The relatively large distances between these groups reflects the fact that Eurasian, North African, Oceanian, and American populations all share significant drift that they do not share with the Sub-Saharan African samples.
