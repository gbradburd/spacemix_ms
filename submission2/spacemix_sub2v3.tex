...
%DIF ADD /Users/gburd/Dropbox/space.mix/ms/submission2/spacemix_plos_template_sub2.tex   Fri Oct 30 11:38:08 2015
...
\DIFaddend We also examined the effects of uneven sampling on inference by 
\DIFdelbegin \DIFdel{sampling random subsets of the simple lattice and barrier scenarios shown in Figs.\ \ref{sfig:lattice_scenarios}}%DIFDELCMD < \subref{simple_lattice} %%%
\DIFdel{and \ref{sfig:lattice_scenarios}}%DIFDELCMD < \subref{barrier_lattice}%%%
\DIFdelend \DIFaddbegin \DIFadd{subsampling a $9\times9$ grid into a variety of subsets that had successfully greater `uneven-ness' of sampling,
and comparing PCA and SpaceMix on these unevenly subsampled datasets}\DIFaddend .
The results of these analyses are \DIFdelbegin \DIFdel{given }\DIFdelend \DIFaddbegin \DIFadd{shown }\DIFaddend in Figs.\ \DIFdelbegin \DIFdel{\ref{sfig:uneven_sampling}}%DIFDELCMD < \subref{lattice} %%%
\DIFdel{and \ref{sfig:uneven_sampling}}%DIFDELCMD < \subref{barrier}%%%
\DIFdel{.}\DIFdelend \DIFaddbegin \DIFadd{\ref{sfig:uneven_sampling_total_grid}--\ref{sfig:uneven_sampling_grid_subsamp_diag}.  
...
Furthermore}\DIFdelend \DIFaddbegin \DIFadd{PCA is a general-purpose tool for exploratory visualization of high-dimensional data;
...
.  
However}\DIFaddend , because PCs are linear functions of the genotypes,
...
may suffer from the same problem\DIFdelbegin \DIFdel{(although we note that }\DIFdelend \DIFaddbegin \DIFadd{, which SpaceMix avoids
(although }\DIFaddend PCA and SPA \DIFdelbegin \DIFdel{both have significant speed advantages over }\DIFdelend \DIFaddbegin \DIFadd{are both significantly faster than }\DIFaddend SpaceMix).  
\DIFaddbegin \DIFadd{Similarly, unevenness of sampling can greatly distort PC maps, 
...
\DIFdelbegin \DIFdel{In comparison, if isolation by distance holds, 
then the two dimensions in which SpaceMix infers geogenetic positions 
...
}\DIFdelend \DIFaddbegin \DIFadd{In this way, the generality of PCA is also its weakness: 
...
\DIFaddend The application of SpaceMix to humans \DIFdelbegin \DIFdel{nicely }\DIFdelend illustrates the utility of our approach: 
...
This comparison is also \DIFdelbegin \DIFdel{nicely illustrated by the example in Fig}\DIFdelend \DIFaddbegin \DIFadd{illustrated in Figs}\DIFaddend .\ \ref{sfig:line_scenario} \DIFaddbegin \DIFadd{and \ref{sfig:uneven_sampling_grid_subsamp_diag}}\DIFaddend .
...
\DIFdelbegin \DIFdel{the }\DIFdelend interpretation and visualization of these higher dimensions \DIFdelbegin \DIFdel{would prove }\DIFdelend \DIFaddbegin \DIFadd{is more }\DIFaddend difficult,
and so \DIFdelbegin \DIFdel{for the moment we stick with }\DIFdelend \DIFaddbegin \DIFadd{we have stuck to }\DIFaddend two dimensions.
